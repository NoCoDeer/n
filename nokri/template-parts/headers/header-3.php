...
$full_width = '';
$is_full_width = isset($nokri['menu_full_width']) ? $nokri['menu_full_width'] : false;
if ($is_full_width == true && $dashboard_fluid != '') {
    $full_width = '<div class="container' . esc_attr($dashboard_fluid) . '">';
} else if ($is_full_width == false && $dashboard_fluid = '') {
    $full_width = '<div class="container-fluid' . esc_attr($dashboard_fluid) . '">';
} else if ($is_full_width == true) {
    $full_width = '<div class="container-fluid">';
} else {
    $full_width = '<div class="container">';
}
?>
<!-- nokri-black-header-start -->
<div class="transparent-header-1 nokri-black-header">
    <nav id="menu-1" class="mega-menu fa-change-black n-header-trabsparent <?php echo esc_attr($dashboard_class) . " " . esc_attr($bread_class); ?>"<?php
    if (wp_basename(get_page_template()) != 'page-home.php' && $page_check == false) {
        echo esc_attr(str_replace('\\', "", $bg_url));
    }
    ?>>
        <section class="menu-list-items ">
            <?php echo nokri_returnEcho($full_width); ?>
            <ul class="menu-logo">
                <li>
                    <a href="<?php echo esc_url(home_url('/')); ?>"><?php echo '<img  src="' . esc_url($logo) . '" class="img-responsive" alt="' . esc_attr__("logo", "nokri") . '" >'; ?></a>
                </li>
            </ul>
                    ...
                    if (is_user_logged_in()) {
                        ?>
                         <li class="profile-pic dropdown">
                            <a href="javascript:void(0)" class="dropdown-toggle" data-bs-toggle="dropdown" role="button" aria-expanded="false">
                                <img src="<?php echo esc_url($image_dp_link[0]); ?>" alt="<?php echo esc_attr__("image", "nokri") ?>" class="rounded-circle" width="36">
                            </a>
                            <ul class="dropdown-menu">
                                <li>
                                    <a class="dropdown-item" href="<?php echo esc_url(get_the_permalink($dashboard_id)); ?>">
                                        <i class="fa fa-user"></i> <?php echo esc_html__('Dashboard', 'nokri'); ?>
                                    </a>
                                </li>
                                <li>
                                    <a class="dropdown-item" href="<?php echo esc_url(wp_logout_url(home_url())); ?>">
                                        <i class="fa fa-power-off"></i> <?php echo esc_html__('Logout', 'nokri'); ?>
                                    </a>
                                </li>
                            </ul>   
                         </li>
                        <?php
                        $usercheck = get_user_meta($current_id, '_sb_reg_type', true);
                        $memberData = get_user_meta($current_id, '_sb_is_member', true);
                        $Permissions = get_user_meta($current_id, 'member_permissions', true);
                        $userPermiss = isset($Permissions['ad_post']) && $Permissions['ad_post'] != '';
                        if ($current_id == $usercheck) {
                            echo '<li><a href="' . get_the_permalink($job_link) . '"class="btn n-btn-flat"><i class="' . esc_attr($bnt_class) . '"></i>' . esc_html($job_button) . '</a></li>';
                        } else if ($memberData != '' && $userPermiss != '') {
                            echo '<li><a href="' . get_the_permalink($job_link) . '"class="btn n-btn-flat"><i class="' . esc_attr($bnt_class) . '"></i>' . esc_html($job_button) . '</a></li>';
                        } else if ($memberData != '' && $userPermiss == '') {
                        ...
                        ?>               
                        <li>
                            <a href="<?php echo get_the_permalink($signin); ?>" class="n-login-header right"><?php echo esc_html__('Login', 'nokri'); ?></a>
                        </li>          
                        <li>
                            <a href="<?php echo get_the_permalink($signup); ?>" class="n-login-header right"><?php echo esc_html__('Register', 'nokri'); ?></a>
                        </li>
                         <li>
                            <a href="<?php echo get_the_permalink($wo_link); ?>" class="btn n-btn-flat"><i class="<?php echo esc_attr($wo_icon); ?>"></i><?php echo esc_html($wo_txt); ?></a>
                        </li>
                        <?php
                    }
                }
