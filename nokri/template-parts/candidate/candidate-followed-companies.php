...
{
	$company_name =  sanitize_text_field($_GET['comp_title']);
}
$order = 'DESC';
if(isset($_GET['comp_order']))
 {
     $order 		  =  sanitize_text_field($_GET['comp_order']);
 }
 ?>
      ...
      <form role="search"  id="company_form" method="get" class="searchform">
        ...
        <input type="hidden" name="candidate-page" value="followed-companies" >
            <div class="form-group position-relative">
                ...
                <input type="text" name="comp_title" value="<?php echo esc_attr($company_name); ?>"  class="form-control" placeholder="<?php echo esc_attr__('Keyword or Name', 'nokri' ); ?>">
...
$company_tot_jobs  = ( count_user_posts( $company_id , 'job_post' ) );	 
 ...
 <div class="posted-job-list resume-on-jobs" id="company-box-<?php echo esc_attr($company_id);?>">
    <ul class="list-inline">
        <li class="resume-id"><?php echo esc_html($count); ?></li>
        <li class="posted-job-title">
        <?php if (esc_url($image_link[0])) { ?>
            <div class="posted-job-title-img">
                 <a href="<?php echo esc_url(get_author_posts_url($company_id)); ?>"><img src="<?php echo esc_url($image_link[0]); ?>" class="img-responsive img-circle" alt="<?php echo esc_attr__('company Image', 'nokri'); ?>"></a>
            </div> 
            <?php } ?> 
            <div class="posted-job-title-meta">
                <a href="<?php echo esc_url(get_author_posts_url($company_id));  ?>"><?php echo esc_html(the_author_meta( 'display_name', $company_id )); ?> </a>
                <p><?php echo esc_html(get_user_meta($company_id, '_user_headline', true )); ?></p>
            </div>
        </li>
       <li class="posted-job-expiration"><a href="<?php echo esc_url(get_author_posts_url($company_id));  ?>"><?php echo esc_html($company_tot_jobs); ?></a> </li>
        <li class="posted-job-action"> 
        <?php wp_nonce_field('unfollow_comp_nonce_action', 'unfollow_comp_nonce') ?>
