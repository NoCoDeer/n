            ...
            <div class="row">
                ...
                <div class="col-md-5 col-xs-12 col-sm-4">
                    <div class="form-group">
                        <form role="search"  id="job_search" method="get" class="searchform position-relative">
                            <input type="hidden" name="candidate-page" value="jobs-applied" >
                            <input type="text" class="form-control" name="job_name" value="<?php echo esc_attr($job_name); ?>" placeholder="<?php echo esc_attr__('Keyword or name', 'nokri'); ?>">
                            <a href="javascript:void(0)" class="a-btn no-top search_aplied_job position-absolute top-0"><i class="ti-search"></i></a>
        ...
        <div class="dashboard-posted-jobs">
            <div class="posted-job-list jobs-saved header-title">
                <ul class="list-inline">
                    ...
                    <?php
                    if ((isset($nokri['job_apply_retr_reapl'])) && $nokri['job_apply_retr_reapl'] == true)
                    {
                    ?>
...
//pagination
            ...
            if ($query->have_posts()) {
                ...
                while ($query->have_posts()) {
                    ...
                    $cand_redirect_status = get_post_meta($job_id, '_job_retracted' . $current_id, true);
                    ...
                    <div class="posted-job-list jobs-saved">
                        <ul class="list-inline">
                            ...
                            <li class="posted-job-title">
                                <?php if (esc_url($image_link[0])) { ?>
                                    <div class="posted-job-title-img">
                                        <a href="javascript:void(0)"><img src="<?php echo esc_url($image_link[0]); ?>" class="img-responsive" alt="<?php echo esc_attr__('micheal', 'nokri'); ?>"></a>
                                    </div>
                                <?php } ?> 
                            ...
                            <li class="posted-job-status"> <span class="label label-default"><?php echo esc_html(nokri_job_post_single_taxonomies('job_type', $job_type)); ?></span></li>
                            <li class="posted-job-expiration"><?php echo esc_html($job_date); ?></li>
                            <li class="posted-job-action"> 
                                <a href="javascript:void(0)" class="btn btn-custom view_app" data-value = "<?php echo esc_attr($job_id); ?>" data-bs-toggle="modal"   data-bs-target="#appmodel"><?php echo esc_html__('Details', 'nokri'); ?></a>
                            </li>
                            <?php
                            if ((isset($nokri['job_apply_retr_reapl'])) && ($nokri['job_apply_retr_reapl'] == true))
                            {
                            ...
                            <li class="posted-job-action"> 
                                <?php
                                if(($cand_status == '0') ||  ($cand_status == '1') || ($cand_status == '3') ||  ($cand_status == '4')){
                                ?>
                                <?php wp_nonce_field('retract_my_job_nonce_action', 'retract_my_job_nonce') ?>
                                <a href="javascript:void(0)" class="btn btn-custom retract_my_job" data-value = "<?php echo esc_attr($job_id); ?>"><?php echo esc_html__('Retract', 'nokri'); ?></a>
                                <?php
                                 } 
                                else if(($cand_status == '2') || ($cand_status == '6')){
                                ?>
                                <?php wp_nonce_field('reApply_my_job_nonce_action', 'reApply_my_job_nonce') ?>
                                <a href="javascript:void(0)" class="btn btn-custom reApply_my_job" data-value = "<?php echo esc_attr($job_id); ?>"><?php echo esc_html__('ReApply', 'nokri'); ?></a>
                                <?php
                             }
                              ?>
                            </li>
