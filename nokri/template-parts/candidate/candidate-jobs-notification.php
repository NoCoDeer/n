            ...
            if (count($final_array) > 0) {
                $notification .= '<li>
                                    <div class="notif-single">
                                        <a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '">' . esc_html($company_name) . " " . '</a>' . esc_html__('Posted New Job', 'nokri') . '<a href="' . esc_url(get_the_permalink($job_id)) . '" class="notif-job-title">' . " " . esc_html(get_the_title()) . '</a>
                                    </div>
