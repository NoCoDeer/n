...
if (get_post_type() == 'job_post') {
    ...
    $search_page = isset($nokri['sb_search_page']) && $nokri['sb_search_page'] != '' ? $nokri['sb_search_page'] : '';
    $check_page = isset($nokri['search_page_link_url']) ? $nokri['search_page_link_url'] : 2;
    $redirect_url = get_the_permalink($search_page);

    if ($check_page == 1) {
        echo esc_url(nokri_redirect($redirect_url));
    } else {
        echo esc_url(nokri_redirect($redirect_url));
    }
} else {
    ...
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
        <div class="n-blog-box">
            ...
            <strong>' . esc_html__('Read More', 'nokri') . '</strong>
                    ...
                    <?php } ?>
                    <a href="<?php echo esc_url(get_the_permalink($pid)); ?>" class="read-more"><?php echo esc_html__("Read More", "nokri"); ?></a>
                </div>
            </div>
        </div>
    </div>
