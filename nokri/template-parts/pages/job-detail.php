...
if (count((array) $single_job_badges) > 0) {
    foreach ($single_job_badges as $job_badge => $val) {
        ...
        $style_color = '';
        if ($color != "") {
            $style_color = 'style=" background-color: ' . esc_attr($bg_color) . '; color: ' . esc_attr($color) . ';"';
        }
        $job_badge_text .= '<li><a href="' . esc_url(get_the_permalink($nokri['sb_search_page']) . '?job_class=' . $val) . '" class="job-class-tags-anchor" ' . esc_attr($style_color) . '><span>' . esc_html(ucfirst($job_badge)) . '</span></a></li>';
    }
}
        ...
        <div class="row">
            <div class="col-md-7 col-sm-12 col-xs-12">
                <div class="job-detail-2">
                    ...
                    <div class="single-job-cat">
                        <?php echo wp_kses(nokri_job_categories_with_chlid($job_id), get_allowed_html_tags()); ?>
                    </div>
                    <h2><?php esc_html(the_title()); ?></h2>
                    <div class="job-detail-meta">
                        <ul>
                            <li><i class="fa fa-map-marker"></i><?php echo wp_kses(nokri_job_country($job_id, ''), get_allowed_html_tags()); ?></li>
                            <li><i class="fa fa-clock-o"></i><?php echo esc_html(nokri_job_post_single_taxonomies('job_type', $job_type)); ?></li>
            ...
            if ($user_id == $post_author_id) {
                ...
                <div class="col-md-5 col-sm-12 col-xs-12">
                    <div class="apply-job">
                        <a class="btn btn-theme" href="<?php echo esc_url(get_the_permalink($nokri['sb_post_ad_page'])); ?>?id=<?php echo esc_attr($job_id); ?>" > <i class="ti-pencil-alt"></i><?php echo " " . esc_html__('Edit Job', 'nokri'); ?></a>
                    </div>
                </div>
                <?php
            }
            ...
            if (get_user_meta($user_id, '_sb_reg_type', true) == 0) {
                if ($post_apply_status == 'active') {
                    ...
                    <div class="col-md-5 col-sm-12 col-xs-12">
                        <div class="apply-job">
                            <a class="btn btn-theme apply_job" data-job-id="<?php echo esc_attr($job_id); ?>" data-bs-toggle="modal" data-bs-target="#myModal"><?php echo esc_html__('Apply Now', 'nokri'); ?></a>
                            ...
                            if ((isset($nokri['linkedin_api_key'])) && $nokri['linkedin_api_key'] != '' && (isset($nokri['linkedin_api_secret'])) && $nokri['linkedin_api_secret'] != '' && (isset($nokri['redirect_uri'])) && $nokri['redirect_uri'] != '') {
                                ...
                                echo '<a href="https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=' . esc_attr($linkedin_api_key) . '&redirect_uri=' . esc_attr($redirect_uri) . '&state=' . esc_attr($state) . '&scope=r_emailaddress r_basicprofile" class="btn btn-theme bookmark">' . esc_html__('Apply With LinkedIn', 'nokri') . '</a>';
                            }
                            ?>
                            <?php
                            ...
                            if ($job_bookmark == '') {
                                ?>
                                <a class="btn btn-success tool-tip save_job" data-value="<?php echo esc_attr($job_id); ?>"  title="<?php echo esc_attr__('Save Job', 'nokri'); ?>"> <i class="fa fa-heart"></i></a>
                            <?php } else { ?>
                                <a class="btn btn-success tool-tip saved" disbaled  title="<?php echo esc_attr__('Already Saved', 'nokri'); ?>"> <i class="fa fa-heart-o"></i></a>
                            <?php } ?>
                        </div>
                    </div>
                <?php } else { ?>
