...
$emp_id = get_user_meta($current_user_id, 'account_owner', true);
...
if (count((array) $get_rating) > 0) {
        ...
        foreach ($get_rating as $get_ratings) {
            ...
            <div class="review-box">
                <div class="main-review">
                    <div class="review-avg-rating"> <span> <?php echo ''.esc_html($single_avg); ?></span>
                        <ul class="">
                            <?php
                            for ($i = 1; $i <= 5; $i++) {
                                if ($i <= $single_avg)
                    ...
                    <div class="review-content-box">
                        <div class="review-content-meta">
                            <div class="review-author-name">
                                <h4><?php echo esc_html(strip_tags_content(get_comment_meta($comment_ids, '_rating_title', true))); ?></h4>
                            </div>
                            <div class="review-text-box">
                                <p><?php echo esc_html(strip_tags_content($get_ratings->comment_content)); ?></p>
                            </div>
                            <div class="review-date"> <span class="user-profile"> <a href="<?php echo esc_url(get_author_posts_url($get_ratings->comment_post_ID)); ?>">
                                    ...
                                    ?>
                                </span> <span><?php echo esc_html(date_i18n(get_option('date_format'), strtotime($get_ratings->comment_date))); ?></span>
                                <?php
                                if (!get_comment_meta($comment_ids, '_rating_reply', true)) {
                                    if (isset($_GET['tab-data']) && $_GET['tab-data'] == "my_ratings") {
                                        ?>
                                        <span class="review-toggle-angle collapsed" data-bs-toggle="collapse" data-bs-target="#review-<?php echo ''.($comment_ids); ?>"> <?php echo esc_html__('Reply', 'nokri'); ?> </span>
                                        <?php
                                    }
                                }
                                ?>
                ...
                if (isset($_GET['tab-data']) && $_GET['tab-data'] == "my_ratings") {
                    if (!get_comment_meta($comment_ids, '_rating_reply', true)) {
                        ?>
                        <div class="profile-review-reply-box collapse" id="review-<?php echo ''.esc_attr($comment_ids); ?>" >
                            <form  class="sb-reply-rating-form" id="sb-reply-rating-form" <?php if (isset($nokri['is_demo_mode']) && $nokri['is_demo_mode'] != true) { ?> data-commentid="<?php echo '' . esc_attr($comment_ids); ?>" <?php } ?>>
                                <div class="form-group">
                                    <textarea name="review-reply" class="form-control review-reply-<?php echo ''.($comment_ids); ?>" placeholder="<?php echo esc_attr__('Your reply to this review ', 'nokri'); ?>" cols="20" rows="3" id="review-reply"  data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('This field is required.', 'nokri'); ?>"></textarea>
                                </div>
                                <?php
                                if (isset($nokri['is_demo_mode']) && $nokri['is_demo_mode'] == true) {
                                    ?>
                                    <input type="submit" class="btn n-btn-flat validate-review-form" value="<?php echo esc_attr__('Submit Reply', 'nokri') ?>" id="validate-review-form" disabled>
                                    <?php
                                } else {
                                    ?>
                                    <input type="submit" class="btn n-btn-flat validate-review-form" value="<?php echo esc_attr__('Submit Reply', 'nokri') ?>" id="validate-review-form">
                                    <input type="hidden" name="comment_id" value="<?php echo ''.esc_attr($comment_ids); ?>" />
                                    <input type="hidden" id="rating_reply_nonce" value="<?php echo esc_attr(wp_create_nonce('nokri_rating_reply_secure')) ?>"  />
                                    <?php
                                }
                                ?>
                            </form>
    ...
    if (isset($_GET['tab-data']) && $_GET['tab-data'] == "my_ratings") {
        ?>
        <div class="rating-pagination pagination-lg"> <?php echo wp_kses(review_pagination($pages, $paged), get_allowed_html_tags()); ?> </div>
        <?php
    } else {
