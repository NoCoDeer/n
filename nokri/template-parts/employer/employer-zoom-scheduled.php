    ...
    foreach ($results as $result) {
        ...
        if (is_array($data_arr) && !empty($data_arr)) {
            ...
            $action = '<ul class="list-inline">
                        <li class="tool-tip" title="' . esc_html__('Start Meeting', 'nokri') . '"> <a href="' . esc_url($join_url) . '"  data-applierId="' . esc_attr($cand_id) . '" data-jobid="' . esc_attr($job_id) . ' class="label label-success   start_single_meeting"> <i class="ti-video-camera"></i></a></li>
            ...
            $tr .= '<tr>
                <td>' . esc_html($meeting_id) . '</td>
                <td>' . esc_html($topic) . '</td>
                <td>' . esc_html($zoom_meet_time) . '</td>                   
