    ...
    if (isset($applier_resumes) && count($applier_resumes) > 0) {
        ...
        foreach ($applier_resumes as $resumes) {
            ...
            $noti_html .= '<li>
				...
				<a href="' . esc_url(get_author_posts_url($applier)) . '">' . esc_html($user_name) . '</a>' . " " . esc_html__('Applied To', 'nokri') . '<a href="' . esc_url(get_the_permalink($resumes->post_id)) . '" class="notif-job-title">' . " " . esc_html(get_the_title($resumes->post_id)) . '</a>
				</div>
				<span class="notif-timing"><i class="icon-clock"></i> ' . esc_html($apply_date) . '</span>
			</li>';
        }
    }
} else {
...
if (isset($_GET['tab-data']) && $_GET['tab-data'] != "") {
    $tab_data = sanitize_text_field($_GET['tab-data']);
}
/* Change Col Size */
$is_show_section = true;
if (isset($_GET['tab-data']) && ($_GET['tab-data'] == "active-jobs" || $_GET['tab-data'] == "pending-jobs" || $_GET['tab-data'] == "inactive-jobs" || $_GET['tab-data'] == "resumes-list" || $_GET['tab-data'] == "candidate-request")) 
{
    $dashboardclass = 'col-xxl-10 col-lg-10 col-md-12 col-sm-12';
    $is_show_section = false;
} else {
    $dashboardclass = 'col-xxl-7 col-lg-7 col-md-12 col-sm-12 order-lg-1 order-md-2';
}
/* top bar class check */
        ...
        if (!empty($cand_ids)) {
            foreach ($cand_ids as $key => $candidate_id) {
                ...
                foreach ($job_alert as $key => $candidate_alert) {
                    ...
                    $noti_html_alert .= ' <li>
				<div class="notif-single">
				<a href="' . esc_url(get_author_posts_url($user_id)) . '">' . esc_html($user_display_name) . '</a>' . " " . esc_html__('have activate Job Alerts on', 'nokri') . '<a href="' . esc_url(get_the_permalink($user_alert_cat)) . '" class="notif-job-title">' . " " . esc_html($term_name) . '</a>
				</div>
				<span class="notif-timing"><i class="icon-clock"></i> ' . esc_html($apply_datesss) . '</span>
			</li>';
                }
            }
        }
...
?> 
<section class="dashboard-new emp_dashboard pt-2 <?php echo esc_attr($emp_dashboard); ?> <?php echo esc_attr($top_bar_class); ?>">
    <div class="container-fluid">
        <div class="row">
            <!-- <div class="col-md-12 col-sm-12 nopadding"> -->
                <!-- <div class="row"> -->
                <div class="col-lg-2 col-md-2 col-sm-2 col-12 nopadding">
                ...
                <?php if ($is_show_section) { ?>
                    <div class="col-lg-3 col-md-12 col-sm-12 col-12 order-lg-2 order-md-1 nopadding" id="dashboard-bar-right">
                        
                            <div class="main-profile-card">
                                <div class="contact-box">
                                    <div class="contact-img">
                                        <img src="<?php echo esc_url($image_link[0]); ?>" id="emp_dp" class="img-responsive" alt="<?php echo esc_attr__('image', 'nokri'); ?>">
                                    </div>
                                    <div class="contact-caption">
                                        ...
                                        ?>
                                        <h4><?php echo esc_html(the_author_meta('display_name', $user_id)); ?></h4>
                                        <span><?php echo esc_html(get_user_meta($user_id, '_user_headline', true)); ?></span>
                                    </div>
                                    <ul class="social-links list-inline">
                                        ...
                                        <?php } if ($emp_google) { ?>
                                            <li> <a href="<?php echo esc_url($emp_google); ?>" target="_blank"><img src="<?php echo esc_url(get_template_directory_uri() . '/images/icons/003-google-plus.png'); ?>" alt="<?php echo esc_attr__('icon', 'nokri'); ?>"></a></li>
                                        <?php } if ($emp_twitter) { ?>
                                        ...
                                        <?php } if ($emp_linkedin) { ?>
                                            <li> <a href="<?php echo esc_url($emp_linkedin); ?>" target="_blank"><img src="<?php echo esc_url(get_template_directory_uri() . '/images/icons/005-linkedin.png'); ?>" alt="<?php echo esc_attr__('icon', 'nokri'); ?>"></a></li>
                                        <?php } ?>
                                    </ul>
                                </div>
                                            ...
                                            <li>
                                                ...
                                                if (isset($notijobs) && $notijobs) {
                                                    if (isset($member_id) && $member_id != '') {
                                                        ...
                                                        ?>
                                                        <div class="notif-single">
                                                            <a href="<?php echo esc_url(get_the_permalink()); ?>?tab-data=resume-notification"><?php echo esc_html__('View all notifications', 'nokri'); ?> </a>
                                                        </div>
                                                        <?php
                                                    }
                                                }
