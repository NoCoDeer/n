...
if (isset($applier_resumes) && count($applier_resumes) > 0) {
    foreach ($applier_resumes as $resumes) {
        ...
        $noti_html .= ' <li>
                    ...
                    <a href="' . esc_url(get_author_posts_url($applier)) . '">' . esc_html($user_display_name) . '</a>' . " " . esc_html__('Applied To', 'nokri') . ' <a href="' . esc_url(get_the_permalink($resumes->post_id)) . '" class="notif-job-title">' . " " . esc_html(get_the_title($resumes->post_id)) . '</a>
		</div>
...
if (current_user_can('editor') || current_user_can('administrator')) {
    ...
    if (!empty($cand_ids)) {
		...
		<span class="notif-timing"><i class="icon-clock"></i> ' . esc_html($job_alert_date) . '</span>
		</li>';
            }
        }
    }
