...
$cand_status = '';
if (isset($_GET['id'])) {
    $job_id = sanitize_text_field($_GET['id']);
}
...
$cand_loc = isset($_GET['cand_loc']) ? sanitize_text_field($_GET['cand_loc']) : "";
$cand_name = isset($_GET['cand_name']) ? sanitize_text_field($_GET['cand_name']) : "";
$cand_gender = isset($_GET['cand_gender']) ? sanitize_text_field($_GET['cand_gender']) : "";
...
if (isset($_GET['c_status'])) {
    ...
    if (isset($_GET['c_status']) && $_GET['c_status'] != "") {
        $c_status = sanitize_text_field($_GET['c_status']);
...
if (count($applier_resumes) != 0) {
    ...
    if ($authors) {
        ...
        foreach ($authors as $author) {
            ...
            $cand_redirect_status = get_post_meta($job_id, '_job_retracted' . $candidate_id, true);
            ...
            if ($cand_status == '0') {
                ...
                $label_class = 'success';
                $counter_active = "counter-active";
            } elseif ($cand_status == '6') {
                $label_class = 'primary';
                $counter_active = "counter-active";
                // if($cand_redirect_status == '1'){
                $cand_final='Retracted';
                // }
            }
            ...
            <a href="javascript:void(0)"    data-bs-toggle="modal" data-bs-target="#zoom_modal"><i class="fa fa-video-camera"></i></a></li>';
            }
            $resume_table .= '  <tr>
                <td>' . esc_html($sr_no) . '</td>
                 <td>
            <div class="posted-job-title-img gt">
                <a href="' . esc_url(get_author_posts_url($candidate_id)) . '"><img src="' . esc_url($image_dp_link[0]) . '" class="img-responsive img-circle" alt="' . esc_attr__('Candidate Image', 'nokri') . '"></a>
            </div> 
            <div class="posted-job-title-meta">
                <a  href="' . esc_url(get_author_posts_url($candidate_id)) . '" target="_blank"  class="cand-view-prof" data-cand_status="' . esc_attr($cand_status) . '"  data-cand_id = "' . esc_attr($candidate_id) . '" data-job_id = "' . esc_attr($job_id) . '">' . esc_html($author->display_name) . '</a>
            ...
            <ul class="list-inline">
                ' . ($cand_status != '6' ? '
                    <li class="tool-tip" title="' . esc_attr__('Take Action', 'nokri') . '"> 
                        <a href="javascript:void(0)" class="label candidate_resume_action" data-applierId="' . esc_attr($candidate_id) . '" data-jobid="' . esc_attr($job_id) . '"  data-bs-toggle="modal" data-bs-target="#myModal"><i class="la la-files-o"></i></a>
                    </li>
                ' : '') . '
                <li class="tool-tip candidate_short_det" title="' . esc_attr__('Application Details', 'nokri') . '" data-applierId="' . esc_attr($candidate_id) . '" data-jobid="' . esc_attr($job_id) . '" data-attachid="' . esc_attr($attachment_id) . '" >
                    <a href="" class="label" data-bs-toggle="modal" data-bs-target="#short-detail-modal" ><i class="la la-edit"></i></a>
                </li>
                ' . $video_pop . '
    ...
    <div class="dashboard-job-filters">  
            ...
            <form  method="get" id="applier_filter_form" >
                ...
                <div class="col-md-4 col-xs-12 col-sm-3">
                    <div class="form-group custom-form-group">
                        ...
                        <input type="text" name="cand_name" class="form-control" placeholder="<?php echo esc_attr__('Name', 'nokri'); ?>" value="<?php echo ''.esc_attr($cand_name); ?>">
                ...
                <div class="col-md-4 col-xs-12 col-sm-3">
                    <div class="form-group">
                        ...
                        <select name="cand_loc" class="select-generat form-control applier_filter_select">
                            ...
                            <?php echo wp_kses(nokri_return_taxanomy_options('ad_location', $cand_loc, false), get_allowed_html_tags()); ?>
                        </select>
                    </div>
                </div>
                <div class="col-md-4 col-xs-12 col-sm-3">
                    <div class="form-group">
                        ...
                        <select  class="select-generat form-control applier_filter_select" name="cand_gender" <?php echo esc_attr(nokri_feilds_operat('cand_gend_setting', 'required')); ?>>
                ...
                <?php echo wp_kses(nokri_form_lang_field_callback(true), get_allowed_html_tags()); ?>
            </form>
        </div>
        <table class="cand-status-filter">    
            <tr>
                ...
                ?>" ><a   href="<?php echo nokri_change_url('c_status', 6); ?>"><?php echo esc_html__('Retracted', 'nokri') . " ( " . show_cand_counter(6) . " )"; ?> </a> </td>         
            </tr>    
        </table> 
    </div>
    <div class="main-body">
