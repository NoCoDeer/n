...
if (isset($_GET['cat-id']) && $_GET['cat-id'] != "" && is_numeric($_GET['cat-id'])) {
    $term_id = sanitize_text_field($_GET['cat-id']);
	 	...
	 	<div class="skin-minimal">
			<form method="get" action="' . get_the_permalink($nokri['sb_search_page']) . '" class="custom-search-form">';
                    ...
                    if (isset($r['types']) && $r['types'] == 1) {
                     ...
                     <input type="text"  class="form-control" name="' . esc_attr($fieldName) . '" placeholder="' . esc_attr($r['titles']) . '" >
				...
				<div class="form-group form-action">
                    ...
                    if (isset($r['types']) && $r['types'] == 2) {
                        ...
                        $customHTML .= '<div class="form-group"><select name="' . esc_attr($fieldName) . '" class="questions-category" >' . $options . '</select></div><input type="submit" class="btn n-btn-flat btn-mid" value="' . esc_html__('Search', 'nokri') . '">';
                    }
                    ...
                    if (isset($r['types']) && $r['types'] == 3) {
                        ...
                        if (isset($r['values']) && $r['values'] != '') {
                            ...
                            foreach ($varArrs as $val) {
                                ...
                                $options .= '<li><input type="radio" id="minimal-checkbox-' . esc_attr($loop) . '"  value="' . esc_html($val) . '" ' . esc_attr($checked) . ' name="' . esc_attr($fieldName) . '"><label for="minimal-checkbox-' . esc_attr($loop) . '">' . esc_html($val) . '</label></li>';
                                $loop++;
                            }
                        }
                    ...
                    if (isset($r['types']) && $r['types'] == 4) {
                        $customHTML .= '<div class="form-group">
                     <input type="text"  class="datepicker-here-dynamic form-control" name="' . esc_attr($fieldName) . '" placeholder="' . esc_attr($r['titles']) . '">
					 <button type="submit" class="btn n-btn-flat btn-mid"><i class="fa fa-search"></i></button>
                </div>
				<div class="form-group form-action">
                </div>';
