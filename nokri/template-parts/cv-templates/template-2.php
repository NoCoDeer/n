...
if($cand_headline != ""){  
$cand_headline  =    '<p></p><p class ="author_headline">'."(".$cand_headline.")".'</p>';
}
if($cand_phone !=""){
...
if ($cand_introd != " ") {
    ...
    $about_sec   .='<div class="wrapper" style="margin-top:2px;">';
    $about_sec .= '<h1 style="font-size:20">' . nokri_feilds_label('cand_about', esc_html__('About me', 'nokri')) . '</h1>';
    $about_sec .= '<p style="font-size:11;line-height:20px">' . $cand_introd . '</p>';
    $about_sec   .='</div>';
}
$cand_dp = "";
if ($image_link != "") {
...
if ($cand_education && $cand_education[0]['degree_name'] != '') {
    ...
    foreach ($cand_education as $edu) {
        ...
        $degre_details     =   strip_tags($degre_details);
...
if (isset($cand_skills) && !empty($cand_skills) && count($cand_skills) > 0) {
    ...
    foreach ($cand_skills as $key => $csv) {
        ...
        if ($term) {
            ...
            $skills_bar .= '<li style="font-size:12;line-height:25px"> ' . $term->name . ' </li>';
        }
    }
    $skills_bar .= "</ul>";
}
...
$html2   =  $about_sec.$skills_bar.$cand_education_html.$cand_profession_html.$cand_certificate_html.$custom_display;
