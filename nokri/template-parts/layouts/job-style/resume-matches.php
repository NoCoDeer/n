...
?>
<tr>
    <td><a class= "job-title" href="<?php echo esc_url(get_the_permalink($job_id)); ?>"><?php esc_html(the_title()); ?></a>
<p><strong><?php echo esc_html__( 'Posted Date', 'nokri' ); ?>: </strong><?php echo esc_html(get_the_date()); ?></p>
<?php echo "".wp_kses($premium_txt, get_allowed_html_tags()); ?>
    </td>
    <td> 
     <span class="label label-<?php echo esc_attr( $staus_clr); ?>"><?php echo esc_html( $job_ap_status); ?></span>
    </td>
    <td> 
     <?php echo esc_html(date_i18n(get_option('date_format'), strtotime($job_expiry))); ?>
    </td>
    <td> 
