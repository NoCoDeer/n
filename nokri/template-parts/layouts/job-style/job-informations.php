    ...
    if (count($taxonomies) > 0) {
        foreach ($taxonomies as $taxonomy) {
            if (in_array($taxonomy->term_id, $job_skills)) {
                ...
                $final_url = esc_url(nokri_page_lang_url_callback($link));
                $skill_tags .= '<a href="' . esc_url($final_url) . '">' . " " . esc_html($taxonomy->name) . " " . '</a>';
            }
        }
    }
}
...
if (get_post_meta($job_id, '_job_attachment', true) != "") {
    ...
    foreach ($portfolios as $portfolio) {
        ...
        if ($filetype['ext'] == 'jpg' || $filetype['ext'] == 'png' || $filetype['ext'] == 'jpeg' || $filetype['ext'] == 'gif') {
            ...
            $job_attachments .= '<li><a href="' . esc_url(wp_get_attachment_url($portfolio)) . '"  target="_blank"><img src="' . esc_url(get_template_directory_uri() . '/images/icons/') . $file_icon . '.png" alt="' . esc_attr__('portfolio image', 'nokri') . '"></a></li>';
        }
    }
}
/* Opening */
...
/* Hide show map on details page */
$job_details_map = isset($nokri['job_details_map']) ? $nokri['job_details_map'] : false;
