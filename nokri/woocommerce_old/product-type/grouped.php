	...
	foreach($all_childs as $child_prod)
	{
		...
		if(isset($_product) && count((array) $_product) > 0 && !empty($_product))
		{
			...
			echo '<li>';
							...
							echo ' <label> <a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink($product_id ), $product_id ) ) . '">' .esc_html($product_name) . '</a>'(.$pricing).'</label></li>';	
