...
?>
        <<?php echo '' . esc_attr($tag); ?> id="comment-<?php esc_attr(comment_ID()); ?>" <?php comment_class($args['has_children'] ? 'parent' : '', $comment); ?>>
                    ...
                    <div class="comment-content">
                        ...
                        <?php if ('0' == $comment->comment_approved) : ?>
                            <p class="comment-awaiting-moderation">
                                <?php esc_html__('Your comment is awaiting moderation.', 'nokri'); ?></p>
                        <?php endif; ?>
                        <?php
                        $reply_link = preg_replace(
                            '/comment-reply-link/',
                            'comment-reply-link ',
                            get_comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'nokri'), 'depth' => $depth, 'max_depth' => $args['max_depth']))),
                            1
                        );
                        ?>
                        <?php
...
if (!function_exists('nokri_top_bar_social_sorter')) {
    ...
    function nokri_top_bar_social_sorter()
    {
        ...
        return ' <ul class="header-social float-start ' . esc_attr($rtl_class) . '">' . $socails . '</ul>';
    }
}
/* ========================= */
...
if (!function_exists('nokri_sm_social_footer_sorter')) {
    ...
    function nokri_sm_social_footer_sorter()
    {
        ...
        return ' <div class="col-md-6 col-sm-6 col-sm-12">
...
function nokri_footer_job_taxonomies_links_($taxonomy, $meta_key, $term_slug, $limit = -1) {
    $args = array(
        'taxonomy'   => $taxonomy,
        'number'     => $limit, // Limit the number of job locations fetched
        'hide_empty' => false, // Show locations even if they don't have jobs
    );
...
//for footer 3 skills
if (!function_exists('nokri_foot_job_skills_links')) {

    function nokri_foot_job_skills_links()
    {
        global $nokri;

        $foot_job_skills_links = '';
        $job_skills = isset($nokri['job_skills_links']) ? $nokri['job_skills_links'] : array();

        if (!is_array($job_skills)) {
            $job_skills = explode(',', $job_skills); // Convert a string to an array if needed
        }
        if (!empty($job_skills) && is_array($job_skills)) {
            foreach ($job_skills as $skill) {
                $skill = intval(trim($skill));
                if ($skill <= 0) {
                    continue; // Skip invalid or empty IDs
                }
                $term = get_term($skill, 'job_skills');
                
                if (is_wp_error($term) || !$term) {
                    // Skip invalid or empty terms
                    print_r("Error with term ID: $skill\n"); // Debug
                    continue;
                }

                // Get the term link and name for each valid job skill
                $term_link = get_term_link($skill, 'job_skills');
                if (!is_wp_error($term_link)) {
                    $term_name = $term->name;
                    $foot_job_skills_links .= '<li> <a href="' . esc_url($term_link) . '">' . esc_html($term_name) . ' </a> </li>';
                }
            }
        }

        return $foot_job_skills_links;
    }
}
        ...
        if ($layout) {
            foreach ($layout as $key => $value) {
                switch ($key) {
                    ...
                    case 'cat':
                        echo '<div class="col-lg-2 col-md-4 col-sm-12">
                                <div class="footer_block">
                                    ...
                                    <ul class="footer-links">
                                    ' . nokri_footer_job_taxonomies_links('job_category', 'job_categories_links', 'cat_id', 5) . '
                                    </ul>
                                </div>
                            </div>';
                        break;
                        ...
                        /* Location case */
                    /* Modify the 'loc' case to fetch and display only 5 job locations */
                    case 'loc':
                        echo '<div class="col-lg-2 col-md-4  col-sm-12">
                                <div class="footer_block">
                                    ' . nokri_footer_input_texts('job_locations_links_text', '', 'h4', '') . '
                                    <ul class="footer-links">
                                    ' . nokri_footer_job_taxonomies_links('ad_location', 'job_locations_links', 'job-location', 5) . '
                                    </ul>
                                </div>
                            </div>';
                        break;
                    ...
                    case 'app':
                        echo '<div class="col-xxl-4 col-md-6  col-sm-12">
                                        <div class="footer_block"> 
                                          ...
                                          <div class="subscription-form position-relative">
                                            <form onSubmit="return checkVals();">
                                             ...
                                             <button id="save_email" type="button"  type="submit" class="btn n-btn-flat btn-block position-absolute top-0" style="right:0px;">' . esc_html__('Send', 'nokri') . '<i class="ti-angle-right"></i></button>
        ...
        if (1 != $pages) {
            ...
            for ($i = 1; $i <= $pages; $i++) {
                if (1 != $pages && (!($i >= $paged + $range + 1 || $i <= $paged - $range - 1) || $pages <= $showitems)) {
                    $paginationHTML .= ($paged == $i) ? "<li class=\"active\"><span>" . $i . "</li>" : "<li><a href='" . get_pagenum_link($i) . "'>" . $i . "</a></li>";
                }
            }
...
if (!function_exists('nokri_footer_posts')) {
    ...
    function nokri_footer_posts()
    {
        ...
        return '<div class="col-sm-6 col-md-4 col-sm-12">
...
if (!function_exists('nokri_comments_list')) :
    ...
    function nokri_comments_list($comment, $args, $depth)
    {
        ...
        if (get_avatar_url($comment, 44) != "") {
            $img = '<img class="float-start ' . esc_attr($rtl_class) . ' hidden-xs img-circle" alt="' . esc_html__('Avatar', 'nokri') . '" src="' . esc_url(get_avatar_url($comment, 22)) . '" />';
        }
        ?>
    ...
    if (!function_exists('nokri_get_current_url')) {
        ...
        function nokri_get_current_url()
        {
            ...
            return $actual_link = esc_url("http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]");
        }
    }
        ...
        function nokri_notice_for_delete_country()
        {
            ?>
                <div class="notice notice-info">
                    <strong>
                        <p><?php echo esc_html__('If you delete country permanently then all associated states and cities will be deleted with it.', 'nokri'); ?>
                        </p>
                    </strong>
                </div>
            <?php
            ...
            ?>
                <div class="notice notice-info">
                    <p><?php echo esc_html__('Must need to aad country NAME as google list like United Arab Emirates, check it', 'nokri'); ?>
                        <a href="<?php echo esc_url('https://developers.google.com/public-data/docs/canonical/countries_csv'); ?>" target="_blank">
    ...
    if (!function_exists('nokri_canidate_apply_status')) {
        ...
        function nokri_canidate_apply_status($getName = '')
        {
            ...
            if ($getName === '') {
                ...
                return isset($arr[$getName]) ? $arr[$getName] : "";
            }
        }
    }
    ...
    /* Candidate Retract Applied Jobs */
    add_action('wp_ajax_del_cand_applied_job', 'nokri_del_candidate_applied_job');
    if (!function_exists('nokri_del_candidate_applied_job')) {
        function nokri_del_candidate_applied_job()
        {

            if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'retract_my_job_nonce_action')) {
                echo '3';
                die();
            }
            global $nokri;
            $user_id = get_current_user_id();
            $job_id = sanitize_text_field($_POST['emp_job_id']);
            update_post_meta($job_id, '_job_applied_status_' . $user_id, 6);
            update_post_meta($job_id, '_job_retracted' . $user_id, 1);
            echo "1";
            die();
        }
    }
    /* Candidate Retract Applied Jobs */
    add_action('wp_ajax_reApply_cand_applied_job', 'nokri_reApply_cand_applied_job');
    if (!function_exists('nokri_reApply_cand_applied_job')) {
        function nokri_reApply_cand_applied_job()
        {
            global $nokri;
            if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'reApply_my_job_nonce_action')) {
                echo '3';
                die();
            }
            $user_id = get_current_user_id();
            ...
            update_post_meta($job_id, '_job_re_apply' . $user_id, 1);
            echo "1";
            die();
        }
    }
    ...
    if (!function_exists('nokri_static_strings')) {
        ...
        function nokri_static_strings()
        {
            ...
            $user_id = get_current_user_id();
            $user_defaul_cv_id = get_user_meta($user_id, '_cand_make_default_resume', true);
            $make_default_option = $nokri["cand_res_make_def"];
            ...
            $signin_page = get_the_permalink($signin_page);
            if ($is_sticky_menu) {
                $is_sticky_menu = true;
            }
            wp_localize_script(
                ...
                array(
                    ...
                    /* For loading */
                    'spiner_working' => esc_html__('Working...', 'nokri'),
                    /* For Education */
                    ...
                    'no_bump_up' => esc_html__('Don not have more bump up', 'nokri'),
                    /* Graphs Chart */
                    'posted_jobs' => esc_html__('Posted Jobs', 'nokri'),
                    'graph_months' => esc_html__('Month', 'nokri'),
                    'jobs_posted' => esc_html__('Jobs Posted', 'nokri'),
                    'post_job_details' => esc_html__('Job Post Details', 'nokri'),
                    'received_res' => esc_html__('Received Resumes', 'nokri'),
                    'res_receiving_date' => esc_html__('Resume Receiving Date', 'nokri'),
                    'total_res' => esc_html__('Total Resumes', 'nokri'),
                    'res_details' => esc_html__('Resume Details', 'nokri'),
                    'job_post_details' => esc_html__('Job Post Details', 'nokri'),
                    'resume_recv_date' => esc_html__('Resume Receiving Date', 'nokri'),
                    'cand_res_details' => esc_html__('Resume Details', 'nokri'),
                    'cand_job_applied' => esc_html__('Jobs Applied ', 'nokri'),
                    'cand_applied_date' => esc_html__('Applied Date', 'nokri'),
                    'cand_applied_jobs' => esc_html__('Applied on Jobs', 'nokri'),
                    'emp_member_added' => esc_html__('Employer Member Added Successfully', 'nokri'),
                    'emp_member_updated' => esc_html__('Employer Member Updated Successfully', 'nokri'),
                    'something_wrong' => esc_html__('Something Went Wrong', 'nokri'),
                    /*Star Ratings and Reviews*/
                    'half_star' => esc_html__('Half Star', 'nokri'),
                    'one_and_half_stars' => esc_html__('One & Half Star', 'nokri'),
                    'two_and_half_stars' => esc_html__('Two & Half Stars', 'nokri'),
                    'three_and_half_stars' => esc_html__('Three & Half Stars', 'nokri'),
                    'four_and_half_stars' => esc_html__('Four & Half Stars', 'nokri'),
                    'one_star' => esc_html__('One Star', 'nokri'),
                    'two_stars' => esc_html__('Two Stars', 'nokri'),
                    'three_stars' => esc_html__('Three Stars', 'nokri'),
                    'four_stars' => esc_html__('Four Stars', 'nokri'),
                    'five_stars' => esc_html__('Five Stars', 'nokri'),
                    'not_rated' => esc_html__('Not Rated', 'nokri'),
                    'cand_make_default' => esc_html__('Make Default', 'nokri'),
                    'cand_default' => esc_html__('Default', 'nokri'),
                    'user_login_text' => esc_html__('Please Login first to View Candidate Profile', 'nokri'),
                    'user_defaul_cv_id' => $user_defaul_cv_id,
                    'make_default_option' => $make_default_option,
                    /* For signin page */
                    'signin_page_url' => $signin_page,
                )
            );
        }
    ...
    /* Rewrite for Categories and Locations for Nokri Job Board */
    add_filter('register_taxonomy_args', 'nokri_change_taxonomies_slug', 10, 2);
    if (!function_exists('nokri_change_taxonomies_slug')) {

        function nokri_change_taxonomies_slug($args, $taxonomy)
        {
            /* item category */
            $nokri_theme_values = get_option('nokri');
            /* Rewrite URL category */
            if (isset($nokri_theme_values['sb_cats_url_rewriting']) && $nokri_theme_values['sb_cats_url_rewriting'] && isset($nokri_theme_values['sb_cat_slug']) && $nokri_theme_values['sb_cat_slug'] != "") {
                if ('job_category' === $taxonomy) {
                    $args['rewrite']['slug'] = $nokri_theme_values['sb_cat_slug'];
                }
            }
            /* Rewrite URL Location */
            if (isset($nokri_theme_values['sb_location_url_rewriting']) && $nokri_theme_values['sb_location_url_rewriting'] && isset($nokri_theme_values['sb_ad_location_slug']) && $nokri_theme_values['sb_ad_location_slug'] != "") {
                if ('ad_location' === $taxonomy) {
                    $args['rewrite']['slug'] = $nokri_theme_values['sb_ad_location_slug'];
                }
            }
            return $args;
        }
    ...
    if (!function_exists('nokri_radius_search_theme')) {
        ...
        function nokri_radius_search_theme($data_arr = array(), $check_db = true)
        {
            ...
            if (is_array($nearby_data) && !empty($nearby_data)) {
                ...
                $distance = (float)$distance; //your distance in KM
                $R = 6371.009; //constant earth radius. You can add precision here if you wish

                $maxLat = (float)$lat + rad2deg($distance / $R);
                $minLat = (float)$lat - rad2deg($distance / $R);
                $maxLon = (float)$lon + rad2deg(asin($distance / $R) / @cos(deg2rad($lat)));
                $minLon = (float)$lon - rad2deg(asin($distance / $R) / @cos(deg2rad($lat)));
    ...
    if (!function_exists('nokri_get_child_lev1')) {
        ...
        function nokri_get_child_lev1()
        {
            ...
            $taxanomy = isset($_POST['tax']) ? sanitize_text_field($_POST['tax']) : "";
    ...
    if (!function_exists('nokri_get_alerts_geolocation_subscription')) {
        ...
        function nokri_get_alerts_geolocation_subscription($user_id = '', $alert_type = array())
        {
            $job_alert = nokri_get_candidates_job_alerts($user_id);
            if (isset($job_alert) && !empty($job_alert)) {
                $terms = array();
                    ...
                    if (isset($job_alert) && !empty($job_alert)) {
                        foreach ($job_alert as $key => $val) {
                            ...
                            if ($date_to_sent == $today && $job_id != '') {
                                ...
                                if (in_array($job_id, $processed_job_ids)) {
                                    continue;
                                } else {
    ...
    //end cron jobs
    //ajx handle for favourite icon
    function add_product_to_favorites()
    {
        if (isset($_POST['product_id'])) {
            $product_id = intval($_POST['product_id']);
            $user_id = get_current_user_id();

            // Add the product to the user's favorites (you can use custom user meta for this)
            ...
            echo esc_html__('success','nokri');

            die();
        }
    }
    add_action('wp_ajax_add_product_to_favorites', 'add_product_to_favorites');
    add_action('wp_ajax_nopriv_add_product_to_favorites', 'add_product_to_favorites');
    /* ============================== */
    /* invoice pricec and description function   */
    if (!function_exists('store_price')) {
        function store_price()
        {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                ...
                $employerid = sanitize_text_field($_POST['employer_id']);
                // Start a session (if not already started)
                if (!session_id()) {
                    session_start();
                }

                // Store the price in a session variable
                $_SESSION['price'] = $price;
                $_SESSION['description'] = $description;
                $_SESSION['rowid'] = $rowid;
                // Send a success response
                $parameters = '?price=' . urlencode($price) .  '&job_id=' . urlencode($jobid) . '&candidate_id=' . urlencode($candidateid) . '&employer_id=' . urlencode($employerid);
                $invoice_page = (isset($nokri['inovice_generate']) && $nokri['inovice_generate'] != "") ? $nokri['inovice_generate'] : '';

                $invoice_generate_page = get_the_permalink($invoice_page);

                $redirectUrl = $invoice_generate_page . $parameters;
                echo esc_url($redirectUrl);
                // Always exit after sending a response in an AJAX callback
                exit();
            }
        }
    }
    // Hook for logged-in users
    add_action('wp_ajax_store_price', 'store_price');
    // Hook for non-logged-in users
    add_action('wp_ajax_nopriv_store_price', 'store_price');
