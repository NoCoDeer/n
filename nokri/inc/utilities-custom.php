...
if (!function_exists('nokri_getBGStyle')) {
    ...
    function nokri_getBGStyle($optname = '')
    {
        ...
        if (isset($bgarea['background-color']) && $bgarea['background-color'] != "") {
            $bg_img .= ' background: ' . esc_attr($bgarea['background-color']) . ';';
        }
        if (isset($bgarea['background-image']) && $bgarea['background-image'] != "") {
            ...
            $bg_img .= ' background: url(' . esc_url($bgarea['background-image']) . '); ';
            $bg_img .= ' background-repeat: ' . esc_attr($bg_repeat) . ';';
            $bg_img .= ' background-size: ' . esc_attr($bg_size) . '; ';
            $bg_img .= ' background-position: ' . esc_attr($bg_position) . '; ';
            $bg_img .= ' background-attachment: ' . esc_attr($bg_attachment) . '; ';
        }
        $bg_img .= '"';
...
if (!function_exists('nokri_make_link')) {
    ...
    {
        return wp_kses("<a href='" . esc_url($url) . "' target='_blank'>", nokri_required_tags()) . esc_html($text) . wp_kses('</a>', nokri_required_tags());
    }
    ...
    function nokri_job_post_taxonomies($taxonomy_name = '', $value = '')
    {
        ...
        if (count((array) $taxonomies) > 0) {
            foreach ($taxonomies as $taxonomy) {
                ...
                $option .= '<option value="' . esc_attr($taxonomy->term_id) . '" ' . esc_attr($selected) . '>' . esc_html($taxonomy->name) . '</option>';
            }
        }
        return $option;
    }
...
if (!function_exists('nokri_job_search_taxonomies_checkboxes')) {
    ...
    function
        nokri_job_search_taxonomies_checkboxes(
        $taxonomy_name = '',
        $instance = array())
    {
        $max_record = $title = '';
        ...
        return '<div class="panel panel-default">
                    <div class="panel-heading ' . esc_attr($cls) . '" role="tab" >
                        <a class="' . esc_attr($collapsed) . '" role="button" data-bs-toggle="collapse" href="#collapse-' . esc_attr($taxonomy_name) . '" >                                                ' . $title . '  
                        </a>
                    </div>
                <div id="collapse-' . esc_attr($taxonomy_name) . '" class="panel-collapse collapse' . esc_attr($is_open) . ' ' . $is_show . '">
...
/* ========================= */
/*   Work Remotely Widget Function  */
/* ========================= */

if (!function_exists('nokri_job_taxonomies_checkboxes')) {

    function
        nokri_job_taxonomies_checkboxes($work_remotely = '',$instance = array())
    {
        $is_open = $instance['is_open'] == 'open' ? 'in' : '';
        if (isset($_GET[$work_remotely]) && $_GET[$work_remotely] != "") {
            $is_open = 'in';
        }
        global $nokri;
        $option = '';
        ...
        $toggle = '';
        $name_job_rmt = 'Work Remotely';
        if ($multi_searach) {

            $option .= '<li class="' . esc_attr($toggle) . '"><input class="' . $search_class . ' change_select" name = "' . $work_remotely . '" type="checkbox" ' . esc_attr($selected) . '><label>' . esc_html($name_job_rmt) . '</label></li>';
        } else {
            $option .= '<li class="' . esc_attr($toggle) . '"><input class="' . $search_class . ' change_select" name = "' . $work_remotely . '" type="radio" ' . esc_attr($selected) . ' ><label>' . esc_html($name_job_rmt) . '</label></li>';
        }
        $cls = $is_open == 'in' ? 'active' : '';
        $collapsed = 'collapsed';
        if ($is_open == 'in') {
        ...
        if ($multi_searach) {
            ...
            $form = '<form  method="get"  action="' . get_the_permalink($nokri['sb_search_page']) . '" >
                 ...
                 ' . nokri_search_params($work_remotely) . '
        </form>';
        }
        return '<div class="panel panel-default">
                    <div class="panel-heading ' . esc_attr($cls) . '" role="tab" >
                        <a class="' . esc_attr($collapsed) . '" role="button" data-bs-toggle="collapse" href="#collapse-' . esc_attr($work_remotely) . '" > ' . $title . '  
                        </a>
                    </div>
                <div id="collapse-' . esc_attr($work_remotely) . '" class="panel-collapse collapse ' . esc_attr($is_open) . '">
                  <div class="panel-body">
                    ' . $form . '                 
                  </div>
                </div>
...
if (!function_exists('nokri_apply_with_external_source')) {
    ...
    function nokri_apply_with_external_source($job_id = '')
    {
        ...
        $href_whatsapp = esc_url("https://api.whatsapp.com/send?phone=$job_apply_whatsapp");
        ...
        if ($job_apply_with == 'exter') {
            ...
            $apply_button = '<a href="' . $href . '" class="btn n-btn-rounded ' . $simple_app . '" '. $data_toggle .' data-target="' . $modal_target . '"  data-job-id=' . ($job_id) . '>' . esc_html($apply_now_text) . ' </a>';
        }
        return $apply_button;
    }
...
if (!function_exists('nokri_get_resume_publically')) {
    ...
    function nokri_get_resume_publically($user_id = '', $type = '')
    {
        ...
        if (!empty($ids_array)) {
            ...
            $download_btn = '<a class="btn n-btn-custom btn-block w-100 mb-2" href="' . esc_url($final_url) . '&download_file=1"><i class="fa fa-download"></i>' . esc_html__('Download Resume', 'nokri') . '</a>';
        }
        if ($type == 'id') {
            return $attach_id;
        } else {
    ...
    function nokri_job_search_taxonomy($id = '')
    {
        ...
        if ((array) $ids && isset($ids[0]->term_id) && $ids[0]->term_id != "") {
            foreach ($ids as $idz) {
                ...
                return '<a href="' . get_the_permalink($nokri['sb_search_page']) . '?job_type=' . $idz->term_id . '" class="badge part-time"  style= "' . ($style_color) . '">' . esc_html($idz->name) . '</a>';
...
if (!function_exists('nokri_term_child')) {
    ...
    function nokri_term_child()
    {
        ...
        foreach ($terms as $term) {
            $taxo .= '<option value="' . esc_attr($term->term_id) . '">' . esc_html($term->name) . '</option>';
        }
...
if (!function_exists('nokri_get_sub_cats_search')) {
    ...
    function nokri_get_sub_cats_search()
    {
        ...
        if (count((array) $ad_cats) > 0) {
            $res .= '<label>' . esc_html($heading) . '</label>';
        ...
        if (isset($array_skills) && !empty($array_skills)) {
            foreach ($array_skills as $r) {
                ...
                if ($skill_as_tag) {
                    ...
                    $skills_bar .= '<a href="' . esc_url($link) . '" class="skills_tags" target="_blank"> ' . esc_html($r["name"]) . '</a>';
                } else {
                    $skills_bar .= '<div class="bar-wrapper">
                    <span class="progress-text">' . esc_html($r["name"]) . '</span>
                    <div class="progress">
                            <div class="progress-bar" role="progressbar" aria-valuenow="' . esc_attr($r["value"]) . '" aria-valuemin="0" aria-valuemax="100"> <span  class="popOver skill_pop" data-bs-toggle="tooltip" data-bs-placement="top" title="' . esc_attr($r["value"]) . '%"> </span> 
                            </div>
                    </div>
                </div>';
                }
...
if (!function_exists('nokri_candidates_get_grid_layouts')) {
    ...
    function nokri_candidates_get_grid_layouts($cand_id = '', $layout = '')
    {
        ...
        $cand_profile_url = esc_url(get_author_posts_url($cand_id));
        if ($restrict_check && ($user_id == "" || $user_id == 0)){
            $sb_user_login = 'sb_user_login';
            $cand_profile_url = "javascript:void(0)";
        }
        ...
        $fav = get_user_meta($u_id, '_emp_saved_resume_' . $u_id, true);
        
        // Ensure $fav is an array (unserialize if needed)
        if (!is_array($fav)) {
            $fav = !empty($fav) ? explode(',', $fav) : []; // Convert comma-separated string to array or default to an empty array
        }

        
        $link_icon = in_array($cand_id, $fav) ? 'fa-heart' : 'fa-heart-o';
        /* Check if the candidate is saved in favorites */
        $link = '<a href="javascript:void(0)" class="bookmark-icon active saving_resume" data-cand-id="' . esc_attr($cand_id) . '"><i class="fa ' . esc_attr($link_icon) . '"></i></a>';
        $link_else = '<a href="javascript:void(0)"><i class="fa ' . esc_attr($link_icon) . '"></i></a>';
        if ($emp_address) {
        ...
        if ($layout == 1) {
            return $layout = '<div class="col-lg-6 col-md-6 col-sm-12">
                           <div class="n-featured-single">
                              <div class="n-featured-candidates-single-top">
                              '.  wp_nonce_field("saving_resume_nonce_action", "saving_resume_nonce") .'
                                                            '. $link .'
                                                          ' . $featured . '
                              ...
                              <div class="n-candidates-single-bottom">
                                 <a href="' . $cand_profile_url . '" class="' . $sb_user_login . '">' . esc_html__('View Profile', 'nokri') . '</a>
                              </div>
                           </div>
                        </div>';
        } else {
            return $layout = '<div class="col-md-6 col-lg-6 col-sm-12">
                            <div class="candidates-grid">
                                ...
                                <div class="cand-btns">
                                    <ul>
                                        ...
                                        <li class="cand-fav saving_resume" data-cand-id="' . esc_attr($cand_id) . '"> '.  wp_nonce_field("saving_resume_nonce_action", "saving_resume_nonce") .' '. $link_else .'</li>
                                    </ul>
                                </div>
                            </div>
                        </div>';
...
if (!function_exists('nokri_footer_logo')) {
    ...
    function nokri_footer_logo()
    {
        ...
        if (isset($nokri['footer_bg']['url']) && $nokri['footer_bg']['url'] != "") {
            ...
            $footerlogo = '<img class="img-responsive footer-logo" src="' . esc_url(get_template_directory_uri() . '/images/logo-white.png') . '" alt="' . esc_attr__("logo", "nokri") . '" >';
        }
        return $footerlogo;
    }
...
if (!function_exists('nokri_employer_menu_sorter')) {
    ...
    function nokri_employer_menu_sorter($user_id = '')
    {
        ...
        // Check if the Candidate Request option should be shown
        $show_candidate_request = Redux::get_option('nokri', 'show_candidate_request_option');
        if (count((array) $menus) > 0) {
            foreach ($menus as $optn => $valu) {
                if (($optn == 'dashboard' || $optn == 'Dashboard') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(get_the_permalink()) . '"><span class="la la-tachometer"></span>' . esc_html($valu) . '</a></li>';
                }
                if (($optn == 'update_profile' || $optn == 'Udpdate Profile') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($dashboard_update_url)) . '"><span class="la la-edit"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'view_my_profile' || $optn == 'View My Profile') && $valu != "") {
                    $label = isset($valu) ? $valu : esc_html__('View My Profile', 'nokri');
                    $sorter .= '<li><a href="' . esc_url(get_author_posts_url($user_id)) . '"><span class="la la-user"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'my_jobs' || $optn == 'My Jobs') && $valu != "") {
                    ...
                    $sorter .= '<li>
                            <div class="profile-menu-link">
                              <span class="la la-bars"></span>' . esc_html($label) . '<i class="fa fa-angle-down"></i>
                            </div>
                            <ul class="submenu">
                ...
                if (($optn == 'candidate_request' || $optn == 'Candidate Request') && $valu != "" && $show_candidate_request) {
                    $label = isset($valu) ? $valu : esc_html__('Candidate Requests', 'nokri');
                    $candidate_request_url = nokri_set_url_param(get_the_permalink(), 'tab-data', 'candidate-request');
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($candidate_request_url)) . '"><span class="la la-file-pdf-o"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'email_templates' || $optn == 'Email Templates') && $valu != "") {
                    ...
                    $sorter .= '<li>
                            <div class="profile-menu-link">
                              <span class="fa fa-envelope-o"></span>' . esc_html($label) . '<i class="fa fa-angle-down"></i>
                            </div>
                            <ul class="submenu">
                ...
                if (($optn == 'matched_resumes' || $optn == 'Matched Resumes') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($matched_resume_url)) . '"><span class="la la-external-link"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'saved_resumes' || $optn == 'Saved Resumes') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($saved_resume_url)) . '"><span class="la la-file-pdf-o"></span>' . esc_html($label) . '</a></li>';
                }
                if(isset($nokri['emp_cand_invite_job']) && $nokri['emp_cand_invite_job'] == true){
                    if (($optn == 'invited_candidate' || $optn == 'Invited Candidate') && $valu != "") {
                        $label = isset($valu) ? $valu : esc_html__('Invited Candidate', 'nokri');
                        $saved_resume_url = nokri_set_url_param(get_the_permalink(), 'tab-data', 'invited-candidate');
                        $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($saved_resume_url)) . '"><span class="la la-users"></span>' . esc_html($label) . '</a></li>';
                    }
                }
                if (($optn == 'followers' || $optn == 'Followers') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($followers_url)) . '"><span class="fa fa-users"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'my_package' || $optn == 'My Package') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($packages_url)) . '"><span class="la la-list-alt"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'my_rating' || $optn == 'My Ratings') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($packages_url)) . '"><span class="la la-star"></span>' . esc_html($label) . '</a></li>';
                }
                if (($optn == 'my_orders' || $optn == 'My Orders') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($orders_url)) . '"><span class="la la-check-circle"></span>' . esc_html($label) . '</a></li>';
                }
                ...
                if (($optn == 'cart' || $optn == 'Cart') && $valu != "") {
                    if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
                        ...
                        $sorter .= '<li><a href="' . wc_get_cart_url() . '"><span class="la la-shopping-cart"></span>' . esc_html($label) . '</a></li>';
                    }
                }
                ...
                if ($is_access) {
                    ...
                    if (($optn == 'zoom_meeting' || $optn == 'My nokri_zoom_meeting') && $valu != "") {
                        ...
                        $sorter .= '<li>
                            <div class="profile-menu-link">
                              <span class="fa fa-video-camera"></span>' . esc_html($label) . '<i class="fa fa-angle-down"></i>
                            </div>
                            <ul class="submenu">
                ...
                if (($optn == 'logout' || $optn == 'Logout') && $valu != "") {
                    ...
                    $sorter .= '<li><a href="' . wp_logout_url(esc_url(home_url())) . '"><span class="la la-sign-out"></span>' . esc_html($label) . '</a></li>';
                }
            }
        }
        return $sorter;
                ...
                if (isset($membersPermiss['manag_jobs']) && $membersPermiss['manag_jobs'] != '') {
                    if (($optn == 'my_jobs' || $optn == 'My Jobs') && $valu != "") {
                        ...
                        $sorter .= '<li>
                            ...
                            <ul class="submenu">
                              <li><a href="' . esc_url(nokri_page_lang_url_callback($active_job_url)) . '">' . esc_html(' Active Jobs', 'nokri') . '</a></li>
                              <li><a href="' . esc_url(nokri_page_lang_url_callback($inactive_job_url)) . '">' . esc_html('In-Active Jobs', 'nokri') . '</a></li>
                              <li><a href="' . esc_url(nokri_page_lang_url_callback($pending_job_url)) . '">' . esc_html('Pending Jobs', 'nokri') . '</a></li>
                            </ul>
                          </li>';
                    }
                }
    ...
    function nokri_candidate_menu_sorter($user_id = '')
    {
        ...
        if (count((array) $menus) > 0) {
            foreach ($menus as $optn => $valu) {
                ...
                if(isset($nokri['emp_cand_invite_job']) && $nokri['emp_cand_invite_job'] == true){
                    if (($optn == 'jobs_invites' || $optn == 'Jobs Invites') && $valu != "") {
                        $label = isset($valu) ? $valu : esc_html__('Jobs Invites', 'nokri');
                        $jobs_invites = nokri_set_url_param(get_the_permalink(), 'candidate-page', 'jobs-invites');
                        $sorter .= '<li><a href="' . esc_url(nokri_page_lang_url_callback($jobs_invites)) . '"><span class="la la-briefcase"></span>' . ($label) . '</a></li>';
                    }
                } 
...
if (!function_exists('nokri_job_country')) {
    ...
    function nokri_job_country($pid, $icon = '') {
        global $nokri;
        $ad_country = '';
        $ad_country = wp_get_object_terms($pid, array('ad_location'), array('orderby' => 'parent'));
        $all_locations = array();
        foreach ($ad_country as $ad_count) {
            $country_ads = get_term($ad_count);
            $item = array(
        ...
        if (count($all_locations) > 0) {
            ...
            /* Set Page URL */
            $check_page = isset($nokri['search_page_link_url']) ? $nokri['search_page_link_url'] : 2;

            for ($i = $limit; $i >= 0; $i--) {
                $term_link = '';
                if ($check_page == 1) {
                    $term_link = nokri_set_url_param(get_the_permalink($nokri['sb_search_page']), 'job-location', esc_attr($all_locations[$i]['term_id']));
                } else {
                    $term_link = get_term_link($all_locations[$i]['term_id']);
                }
                $final_url = esc_url(nokri_page_lang_url_callback($term_link));

                if ($icon == 'yes') {
                  

                    $location_html .= '<li><a href="' . esc_url($final_url) . '"> <i class="fa fa-map-marker" aria-hidden="true"></i>' . esc_html($all_locations[$i]['location']) . '</a></li>';
                } else {

                 //   $location_html .= '<a href="' . $final_url . '">' . esc_html($all_locations[$i]['location'] . ',') . '</a> ';

                    $location_html .= '<a href="' . esc_url($final_url) . '">' . ' ' . '' . esc_html($all_locations[$i]['location']).'</a>' . ',';
                }
            }
        }
        return rtrim($location_html, ',');
...
/* ================================== */
/*  Return Job Categories    Preferences: Configure Language     */
/* =================================== */
if (!function_exists('nokri_job_categories_with_chlid')) {
    ...
    function nokri_job_categories_with_chlid($pid)
    {
        ...
        foreach ($post_categories as $c) {
            ...
            $final_url = esc_url(nokri_page_lang_url_callback($link));
            $cats_html .= '<a href="' . esc_url($final_url) . '">' . esc_html($cat->name) . '</a>, ';
        }
...
if (!function_exists('nokri_js_redirect')) {
    ...
    {
        echo ("<script>jQuery(document).ready(function($) { window.location = '" . esc_url($url) . "' });</script>");
    }
    ...
    function nokri_get_products_theme_options($for = '')
    {
        ...
        if (!class_exists('WooCommerce')) {
            ...
            $args = array(
                ...
                'orderby' => 'ID',
                'meta_query' => array(
                    array('key' => 'op_pkg_for', 'value' => $for, 'compare' => '=',
                    ),
                )
            );
            ...
            if ($the_query->have_posts()) {
                ...
                while ($the_query->have_posts()) {
                    $the_query->the_post();
                    $packages_arr[get_the_ID()] = get_the_title();
                }
            }
            // Reset Post Data
            wp_reset_postdata();
...
function nokri_remove_before_add_to_cart($cart_item_data)
{
    ...
    foreach (WC()->cart->get_cart() as $cart_item) {
        ...
        $package_exp = get_post_meta($product_id, 'package_expiry_days', true);
        if ($package_exp && $package_exp != '') {
            WC()->cart->empty_cart();
        }
    }
...
/* * ********************************* */
/* Getting candidates skills New Design */
/* * ******************************** */
if (!function_exists('nokri_get_candidates_skills_new')) {

    function nokri_get_candidates_skills_new($user_id = '', $layout = '')
    {
        global $nokri;
        $emp_skills = get_user_meta($user_id, '_cand_skills', true);
        $skill_tags = $total_skills = '';
    ...
    function nokri_get_custom_feilds($author_id = '', $feilds_for = '', $id = '', $edit_profile = '', $show_profile = '')
    {
        ...
        if (is_array($custom_feilds)) {
            foreach ($custom_feilds as $value) {
                ...
                $field_slug = (isset($value->feild_slug) && $value->feild_slug != "") ? $value->feild_slug : preg_replace('/\s+/', '', $field_label);
                ...
                if (!$show_profile) {
                    ...
                    if ($field_type == 'RadioButton') {
                        ...
                        foreach ($field_values as $value) {
                            ...
                            if ($field_req) {
                                $message = 'data-parsley-error-message="' . esc_html__('This field is required.', 'nokri') . '"';
                                $requires = 'data-parsley-required="true" ' . esc_attr($message);
                            }
                          
                            $check_html .= '<li><input type="radio" name="_custom_[' . $field_slug . ']" class="input-icheck-others" ' . esc_attr($checked) . '  value="' . esc_attr($value) . '" ' . esc_attr($requires) . '><p>' . esc_html($value) . '</p></li>';
                        }
                        $custom_feilds_html .= '<div class="col-xxl-12 col-lg-12 col-md-12 col-sm-12">
                                                        <div class="mb-3">
                    ...
                    if ($field_type == 'Input') {
                        ...
                        $custom_feilds_html .= '<div class="col-xxl-12 col-lg-12 col-md-12 col-sm-12"><div class="mb-3">
                            <input placeholder="' . esc_attr($field_label) . '" class="form-control" type="text" ' . esc_attr($requires) . ' name="_custom_[' . $field_slug . ']" value="' . esc_attr($meta_value) . '">
                        </div></div>';
                    }
                    /* Number */
                    if ($field_type == 'Number') {
                        ...
                        $custom_feilds_html .= '<div class="col-lg-12 col-md-12 col-sm-12"><div class="form-group">
                            <input placeholder="' . esc_attr($field_label) . '" class="form-control" type="Number" data-parsley-type="digits" ' . esc_attr($requires) . ' name="_custom_[' . $field_slug . ']" value="' . esc_attr($meta_value) . '">
                        </div></div>';
                    }
                    /* Text Area */
                    if ($field_type == 'Text Area') {
                        ...
                        $custom_feilds_html .= '<div class="col-xxl-12 col-md-12 col-sm-12">
                                                    <div class="mb-3">
                                                        <label class="">' . esc_html($field_label) . '</label>
                                                        <textarea  name="_custom_[' . $field_slug . ']" class="form-control"   cols="30" rows="10" ' . esc_attr($requires) . '>' . esc_html($meta_value) . '</textarea>
                                                    </div>
                                                </div>';
                    }
                    /* Select Box */
                        ...
                        $custom_feilds_html .= '<div class="col-xxl-12 col-lg-12 col-md-12 col-sm-12">
                                                        <div class="mb-3">
                                                            <label>' . esc_html($field_label) . '</label>
                                                                <select class="js-example-basic-single form-select" name="_custom_[' . $field_slug . ']" ' . esc_attr($requires) . '>
                                                                    ' . ($options) . ' 
                                                                </select>
                                                        </div>
                                                    </div>';
                    ...
                    if ($field_type == 'Date') {
                        ...
                        $custom_feilds_html .= '<div class="col-xxl-12 col-md-12 col-sm-12">
                                                        <div class="mb-3">
                                                            <label >' . esc_html($field_label) . '</label>
                                                            <input type="text"   value="' . esc_attr($meta_value) . '" name="_custom_[' . $field_slug . ']" class="datepicker-custom-feilds form-control" ' . esc_attr($requires) . '/>
                                                        </div>
                                                    </div>';
                    }
                } else {
                    if ($field_pub == 'Yes') {
                        ...
                        //$meta_value = get_user_meta($user_id, $field_slug, true);
                        if ($meta_value != '') {
                            $custom_feilds_html .= '<li><small>' . esc_html($field_label) . '</small><strong>' . esc_html($meta_value) . '</li></strong>';
                        } else {
                            $custom_feilds_html .= '';
                        }
                    }
...
if (!function_exists('nokri_duplicate_posts_lang_callback')) {
    ...
    function nokri_duplicate_posts_lang_callback($org_post_id = 0)
    {
        ...
        if (class_exists('Redux')) {
            $_duplicate_post = Redux::get_option('nokri', 'nokri_duplicate_jobs');
        }
...
if (!function_exists('nokri_show_taxonomy_all')) {
    ...
    function nokri_show_taxonomy_all($taxo_id, $taxo_nme)
    {
        ...
        if (class_exists('Redux')) {
            $nokri_show_posts = Redux::get_option('nokri', 'nokri_display_all_lang');
        }
                ...
                <div class="dropup ad-language">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        foreach ($languages as $lang) {
                            ...
                            ?>
                        <li>
                            <a href="<?php echo '' . esc_url($lang_link); ?>">
                                <img
                                    src="<?php echo esc_url($lang['country_flag_url']); ?>"><?php echo icl_disp_language($lang['native_name']); ?>
                            </a>
                        </li>
                        <?php } ?>
                    </ul>
...
if (!function_exists('nokri_can_set_auto_viewed')) {
    ...
    function nokri_can_set_auto_viewed()
    {
        ...
        $cand_status = sanitize_text_field($_POST['cand_status']);
...
$output = preg_replace('@<(' . implode('|', $tags) . ')\b.*?>.*?</\1>@si' , '' , $text); } } elseif ($invert==FALSE) {
    ...
    $html = '<ul class="review-stars">';
        for ($i = 1; $i <= 5; $i++) { if ($i <=$total_avg) $html
            .='<li class="star colored-star"><i class="fa fa-star"></i></li>' ; else $html
            .='<li class="star"><i class="fa fa-star"></i></li>' ; } $html .='</ul>' ; return $html; } } } if
            (!function_exists('review_pagination')) { function review_pagination($total_records, $current_page) {
            //$total_record.'' ; //$current_page.'b'; // Check if a records is set. if (!isset($total_records)) return;
            if (!isset($current_page)) return; $next_text=esc_html__('Next Page »', 'nokri' ); $prev_text=esc_html__('«
            Previous Page', 'nokri' ); $args=array( 'base'=> add_query_arg('paged', '%#%'),
            'format' => '?paged=%#%',
            'total' => $total_records,
            'current' => $current_page,
            'show_all' => false,
            ...
            global $nokri;
            $is_approve = isset($nokri['sb_allow_auto_order_complete']) ? $nokri['sb_allow_auto_order_complete'] :
            false;
            ...
            $selected_cat = isset($_GET['cat-id']) ? sanitize_text_field($_GET['cat-id']) : "";
            $cats_html = nokri_return_taxanomy_options('job_category', $selected_cat, true);

            $selected_loc = isset($_GET['job-location']) ? sanitize_text_field($_GET['job-location']) : "";
            $countries_html = nokri_return_taxanomy_options('ad_location', $selected_loc, true);

            $multi_searach = (isset($nokri['multi_job_search_form']) && $nokri['multi_job_search_form'] != "") ?
            $nokri['multi_job_search_form'] : false;
            ?>

            <li>
                <div class="form-group">
                    <label><?php echo esc_html__('Job Title', 'nokri') ?></label>
                    <input type="text" id="autocomplete-dynamic" autocomplete="off" class="form-control"
                        name="job-title" value="<?php echo esc_attr($job_title); ?>"
                        placeholder="<?php echo esc_attr__('Enter keyword or title', 'nokri'); ?>">
            ...
            <li class="res-toggle">
                <form method="get" action="<?php echo esc_url(get_the_permalink($nokri['sb_search_page'])) ?>">
                    <div class="form-group">
                        <label><?php echo esc_html($taxonomy_title) ?></label>
                        <select class="js-example-basic-single select_submit"
                ...
                <div class="form-group">
                    <div class="form-btn">
                        ...
                        <a href="javascript:void(0)"
                            class="btn n-btn-flat adv_opt"><?php echo esc_html__('Advance', 'nokri'); ?></a>
                    </div>
                </div>
            </li>
            <?php
    ...
    function nokri_return_taxanomy_options($taxanomy_name, $selected_opt, $count = true)
    {
        ...
        foreach ($cat_taxanomy as $cat) {
            ...
            if (count($ad_sub_cats) > 0) {
                ...
                $cats_html .= '<option value="' . esc_attr($cat->term_id) . '"  ' . esc_attr($selected) . '>' . esc_html($cat->name) . esc_html($cat_count);
                $selected_sub = "";
                foreach ($ad_sub_cats as $sub_cat) {
                    ...
                    $cats_html .= '<option value="' . esc_attr($sub_cat->term_id) . '" ' . esc_attr($selected_sub) . '>' . '&nbsp;&nbsp; - &nbsp;' . esc_html($sub_cat->name) . esc_html($cat_count) . '</option>';
                    //sub sub cat
                    ...
                    if (count($ad_sub_sub_cats) > 0) {
                        foreach ($ad_sub_sub_cats as $sub_cat_sub) {
                            ...
                            $cats_html .= '<option value="' . esc_attr($sub_cat_sub->term_id) . '"   ' . esc_attr($selected_sub_sub) . '>' . '&nbsp;&nbsp; - &nbsp; - &nbsp;' . esc_html($sub_cat_sub->name) . esc_html($cat_count_sub) . ' </option>';
                            //sub sub sub cat
                            ...
                            if (count($ad_sub_sub_sub_cats) > 0) {
                                foreach ($ad_sub_sub_sub_cats as $sub_cat) {
                                    ...
                                    $cats_html .= '<option value="' . esc_attr($sub_cat->term_id) . '"  ' . esc_attr($selected_sub_sub_sub) . '>' . '&nbsp;&nbsp; - &nbsp; - &nbsp;- &nbsp;' . esc_html($sub_cat->name) . esc_html($cat_sub_sub_count) . '</option>';
                                }
                            }
                        }
                    }
                ...
                $cats_html .= '<option value="' . esc_attr($cat->term_id) . '"  ' . esc_attr($selected) . '>' . esc_html($cat->name) . esc_html($cat_count) . '</option>';
            }
        }
        return $cats_html;
    }
...
if (!function_exists('nokri_send_sms_to_users')) {

    function nokri_send_sms_to_users($receiver_ph)
    {
        global $nokri;
        $custom_msg_btn = isset($nokri['user_custom_msg_btn']) ? $nokri['user_custom_msg_btn'] : false;
        if ($custom_msg_btn) {
            $message = (isset($nokri['user_custom_message']) && $nokri['user_custom_message'] != '') ? $nokri['user_custom_message'] : '';
            $gateway = nokri_verify_sms_gateway();
            if ($gateway == "iletimerkezi-sms") {
                $ilt_data = get_option('ilt_option');
                $options = ilt_get_options();
...
if (!function_exists('nokri_user_avatar_image')) {
    ...
    {
        global $nokri;
        $user_pic = get_template_directory_uri() . '/images/candidate-dp.jpg';
        if (isset($nokri['nokri_user_dp']['url']) && $nokri['nokri_user_dp']['url'] != "") {
            $user_pic = $nokri['nokri_user_dp']['url'];
        }
            ...
            if ($user && is_object($user)) {
                if ($user->data->ID != '') {
                    ...
                    if (isset($image_link) && $image_link != "") {
                        ...
                        if (is_array($headers) && !empty($headers) && strpos($headers[0], '404') === false) {
...
if (!class_exists('dwt_listing_products_shop')) {
    ...
    class dwt_listing_products_shop
    {
        ...
        function dwt_listing_shop_listings_shop_grid($product_id, $column_size = '', $clearfix = '')
        {
            ...
            $additional_div = '';
            if ($column_size == '4') {
                $column_size = 'col-md-4 col-sm-12';
                if ($clearfix != "") {
                    if ($clearfix++ % 3 == 0) {
                ...
                $column_size = 'col-md-3 col-sm-12';
                if ($clearfix != "") {
                    if ($clearfix++ % 4 == 0) {
                        $additional_div = '<div class="clearfix"></div>';
                    }
...
if (!function_exists('nokri_get_jooble_jobs')) {
    ...
    function nokri_get_jooble_jobs()
    {
        ...
        $url = esc_url("https://jooble.org/api/");
        $keywords = isset($_GET['job-title']) && $_GET['job-title'] ? sanitize_text_field($_GET['job-title']) : 'IT';
        $location = isset($_GET['job-location']) && $_GET['job-location'] ? sanitize_text_field($_GET['job-location']) : '';
        $data = array(
            'keywords' => $keywords,
            'location' => $location,
        );
...
if (!function_exists('nokri_get_reedco_jobs')) {
    ...
    function nokri_get_reedco_jobs()
    {
        ...
        $password = "";
        $keywords = isset($_GET['job-title']) && $_GET['job-title'] != '' ? sanitize_text_field($_GET['job-title']) : 'web-developer';
        $location = isset($_GET['job-location']) && $_GET['job-title'] ? sanitize_text_field($_GET['job-location']) : '';
        /* Curl Initialization */
        $curl = curl_init();
        curl_setopt_array(
            $curl,
...
if (!function_exists('nokri_import_adzuna_jobs')) {
    ...
    function nokri_import_adzuna_jobs()
    {
        ...
        $keywordss = isset($_GET['job-title']) && $_GET['job-title'] ? sanitize_text_field($_GET['job-title']) : 'web';
        //$locations = isset($_GET['job-location']) && $_GET['job-title'] ? $_GET['job-location'] : 'ca';
        $url = esc_url('http://api.adzuna.com/v1/api/jobs/' . $locKeyword . '/search/1?app_id=' . $apiID . '&app_key=' . $apiKey . '&results_per_page=35&what=' . $keywordss . '&content-type=application/json');
        $curl = curl_init();
        curl_setopt_array(
            $curl,
            array(
...
if (!function_exists('nokri_get_careerjet_jobs')) {
    ...
    function nokri_get_careerjet_jobs()
    {
        ...
        $user_ip = isset($nokri['careerJet_user_ip']) && $nokri['careerJet_user_ip'] ? $nokri['careerJet_user_ip'] : '10.201.169.196';

        $curl = curl_init();
        $url = esc_url('http://public.api.careerjet.net/search?user_agent=');
        curl_setopt_array(
            $curl,
            array(
                CURLOPT_URL => $url . '' . $apikey . '&user_ip=' . $user_ip . '&keywords=' . $keywords . '&location=' . $location . '',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
        ...
        $jobs_data = json_decode($response);
        if (isset($jobs_data->jobs) && !empty($jobs_data->jobs)) {
            $jobs = $jobs_data->jobs;
...
if (!function_exists('nokri_get_muse_jobs')) {

    function nokri_get_muse_jobs()
    {

        global $nokri;

        //$job_category = isset($nokri['muse_jobs_cats_select']) && $nokri['muse_jobs_cats_select'] ? $nokri['muse_jobs_cats_select'] : 'IT';
        $location = isset($_GET['job-location']) && $_GET['job-title'] != '' ? sanitize_text_field($_GET['job-location']) : '';

        $final_array = $eachrecord = $jobs_data = $jobs_data_array = array();
        $curl = curl_init();
        curl_setopt_array(
            $curl,
            array(
                CURLOPT_URL => 'https://www.themuse.com/api/public/jobs?page=1&location=' . $location . '',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
        ...
        $jobs_data = json_decode($response);
        if (!is_object($jobs_data)) {
            return false;
        } else {
            $jobs_data_array = $jobs_data->results;
        }
        foreach ($jobs_data_array as $job) {

            $eachrecord['id'] = $job->id;
            $eachrecord['name'] = $job->name;
            $eachrecord['date'] = $job->publication_date;
            $eachrecord['refs'] = $job->refs->landing_page;
            $job_locations = $job->company;
            $eachrecord['location'] = $job_locations->name;
            $job_cats = $job->categories;
            foreach ($job_cats as $job_cat) {

                $eachrecord['category'] = $job_cat->name;
            }
            $final_array[] = $eachrecord;
        }
    ...
    function nokri_get_total_saved_resume_count($user_id)
    {
        ...
        if (isset($_GET['c_name'])) {
            if (isset($_GET['c_name']) && $_GET['c_name'] != "") {
                $c_name = sanitize_text_field($_GET['c_name']);
            }
        }
        $c_order = 'DESC';
        if (isset($_GET['c_order'])) {
            if (isset($_GET['c_order']) && $_GET['c_order'] != "") {
                $c_order = sanitize_text_field($_GET['c_order']);
            }
        }
...
if (!function_exists('nokri_get_cand_save_jobs_count')) {
    ...
    function nokri_get_cand_save_jobs_count($user_id)
    {
        ...
        if (isset($_GET['job_name'])) {
            $job_name = sanitize_text_field($_GET['job_name']);
        }
        $args = array(
            'post_type' => 'job_post',
            's' => $job_name,
            ...
            'order' => 'DESC',
            'meta_query' => array(
                array('key' => '_job_saved_value_' . $user_id,
                ),
            ),
        );
        $query = new WP_Query($args);
...
if (!function_exists('nokri_pending_jobs_count')) {
    ...
    function nokri_pending_jobs_count($user_id)
    {
        ...
        global $nokri;
        $job_name = (isset($_GET['job_name']) && $_GET['job_name'] != "") ?sanitize_text_field( $_GET['job_name']) : '';
...
if (!function_exists('nokri_get_remotive_jobs')) {
    ...
    $base_url = esc_url("https://remotive.io/api/remote-jobs?");

    // Get the job title from the query parameter and sanitize it
    $keywords = isset($_GET['job-title']) && !empty($_GET['job-title']) ? urlencode($_GET['job-title']) : '';

    // Get the job limit from the configuration, defaulting to 5 if not set
    ...
    $url = $base_url . "search=" . $keywords . "&limit=" . $jobs_limit;
...
if (!function_exists('nokri_count_user_reports_against_job')) {
    ...
    function nokri_count_user_reports_against_job($job_id)
    {
        ...
        global $wpdb;
        $query = "SELECT count(meta_key) as reports_count, meta_value FROM $wpdb->postmeta WHERE post_id = '$job_id' AND meta_key like '_sb_job_report_user_%' ";
        $total_reports = array_reverse($wpdb->get_results($query));
        $reports_counter = "";
...
/* Get User Reviews average Numeric Value */
if (!function_exists('nokri_get_user_rating_average_value')) {

    function nokri_get_user_rating_average_value($user_id)
    {
...
if (!function_exists('nokri_get_client_ip')) {

    function nokri_get_client_ip()
    {
        $ipaddress = '';
