...
if (!function_exists('nokri_getHeader')) {
    ...
    function nokri_getHeader($sb_section_title, $sb_section_description)
    {
        ...
        $header_html = '';
        if ($title || $desc != '') {
            $header_html = '<div class="col-lg-12 col-md-12 col-sm-12">
...
//             // if(isset($url) && isset($title) && isset($target) && isset($rel) )
//             // {
...
if (!function_exists('nokri_extarct_link')) {
    function nokri_extarct_link($string) {
        if ($string != "") {
            ...
            if (count($arr) >= 2) { // At least URL and Title should be present
                $url = urldecode(nokri_themeGetExplode($arr[0], ':', '1'));
                $title = urldecode(nokri_themeGetExplode($arr[1], ':', '1'));

                if (count($arr) >= 3) { // Check for Target
                    $target = urldecode(nokri_themeGetExplode($arr[2], ':', '1'));
                }
                if (count($arr) >= 4) { // Check for Rel
                    $rel = urldecode(nokri_themeGetExplode($arr[3], ':', '1'));
                }

                return array("url" => $url, "title" => $title, "target" => $target, "rel" => $rel);
            }
        }
        return null; // Return null if the provided string is empty or doesn't have enough values
    }
}
