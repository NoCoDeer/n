    ...
    function price_classic_short_base_func($atts, $content = '') {
                    ...
                    if (isset($row['product'])) {
                        ...
                        if (!empty($product)) {
                            ...
                            if ($price_pos == '%1$s%2$s') {
                                $get_price_html = '<h3>' . $product->get_price_html() . '</h3>';
        ...
        if (isset($section_background) && $section_background == '1') {
            ...
            if ($prcing_bg_img != "") {
                ...
                $bg_img = ($bgImageURL != "") ? ' style="background: url(' . $bgImageURL . ') no-repeat; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover; background-position: center center; background-attachment:scroll;"' : "";
            }
        
            $bg_option = $bg_img; // No need for str_replace
            $bg_option_class = '';
            $heading_class = 'white';
        } else {
            $bg_option_class = 'white-bg'; // Fixed spelling from 'whilte-bg'
            $bg_option = '';
            $heading_class = '';
        }
        /* Section title */
