                    ...
                    if (isset($row['product'])) {
                        ...
                        if (!empty($product)) {
                            ...
                            if (isset($row['product_bg_img']) && $row['product_bg_img'] != "") {
                                ...
                                $product_img = ( $product_imgeURL != "" ) ? ' \\s\\t\\y\\l\\e="background:  url(' . $product_imgeURL . ')  no-repeat; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size:cover; background-position: bottom center; background-attachment:scroll;"' : "";
                            }
                            ...
                            if ($count == 2) {
                             ...
                             /* Getting price position package */
                            $price_pos = get_woocommerce_price_format();


                            if ($price_pos == '%1$s%2$s') {
                                $get_price_html = '<span class="n-ex">' . $product->get_price_html() . '</span>';
                            } elseif ($price_pos == '%2$s%1$s') {
                               $get_price_html = '<span class="n-ex">' . $product->get_price_html() . '</span>';
                            } elseif ($price_pos == '%1$s&nbsp;%2$s') {
                                $get_price_html = '<span class="n-ex">' . $product->get_price_html() . '</span>';
                            } else {
                                $get_price_html = '<span class="n-ex">' . $product->get_price_html() . '</span>';
                            }
                            ...
                            if ($is_pkg_free) {
                                ...
                                $price_html = '<div class="nth-pack-primium">' . $get_price_html . '</div>';
                            }
                            ...
                            $html .= '<div class="col-lg-4 col-md-4 col-sm-12">
								...
								<div class="nth-pckg-content">
									' . $price_html . '
									<span class="product-title">' . get_the_title($row['product']) . '</span>
									' . $pkg_details . '
								</div>
