    ...
    function jobs_cand_cat_section_short_base_func($atts, $content = '') {
            ...
            while ($job_class_slider->have_posts()) {
                ...
                $slider_html .= '<div class="n-listing-content">
							...
							<div class="n-listing-usr-information">
								...
								<a href="javascript:void(0)" class=" absolute-position btn n-btn-flat apply_job" data-bs-toggle="modal" data-bs-target="#myModal"  data-job-id =' . esc_attr($job_id) . '>' . esc_html__('Apply Now', 'nokri') . ' </a>
        ...
        if ($recent_job_query->have_posts()) {
            while ($recent_job_query->have_posts()) {
                ...
                {
                    $apply_button = '<a href="'.$href.'" class="absolute-position btn n-btn-flat '.$simple_app.'" data-bs-toggle="modal" data-bs-target="'.$modal_target.'"  data-job-id=' . esc_attr($job_id) . '>' . esc_html($apply_now_text) . ' </a>';
                }
       ...
       /* Tabs Data */
        ...
        $tab_html = '
            <ul class="nav nav-tabs" role="tablist">
                <li class="nav-item">
                    <a class="nav-link active" id="latest-tab" data-bs-toggle="tab" href="#latest" role="tab" aria-controls="latest" aria-selected="true">' . $latest_job_tab . '</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="premium-tab" data-bs-toggle="tab" href="#premium" role="tab" aria-controls="premium" aria-selected="false">' . $premium_job_tab . '</a>
                </li>
            </ul>
        ...
        $cand_slider_title = (isset($slider_title) && $slider_title != "") ? '<div class="n-candidates-content abc"><h2>' . $slider_title . '</h2></div>' : '';
        $cand_slider_html = '';
...
// For Job Category
        ...
        /* Sider bar widgets */
        $side_bar_widgets = '';
        $wrap_class = 'col-lg-12 col-md-8 col-sm-12';
        if ($slider_switch == '1' || $cat_switch == '1' || $qoute_switch == '1') {
            $wrap_class = 'col-lg-8 col-md-8 col-sm-12';
