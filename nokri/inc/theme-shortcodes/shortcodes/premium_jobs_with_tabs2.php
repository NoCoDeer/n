	...
	if ( (array) count( $rows ) > 0 ) { 
		...
		foreach( $rows as $row ) { 
			...
			$tabs_html .= '<li class="nav-item">
								<a class="nav-link '.esc_attr($active).'" data-bs-toggle="tab" href="#home'.$count.'">'.$term->name.'</a>
						   </li>';
	...
	return  '<section class="n-jobs-recomend3 space3">
				<div class="container">
					<div class="row clear-custom">
						<div class="col-xxl-12 col-lg-12 col-sm-12 col-md-12 position-relative">
							<div class="n-listing-text">
							'.$section_title.'
							'.$section_description.'
							</div>
							
							<ul class="nav nav-tabs custom-nav position-absolute right-0">
								'.$tabs_html.'
							</ul>
