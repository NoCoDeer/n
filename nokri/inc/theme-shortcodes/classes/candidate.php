...
function nokri_candidate_profile() {
    ...
    if(empty($params['cand_person_save_nonce']) || !wp_verify_nonce($params['cand_person_save_nonce'], 'cand_person_save_nonce_action')) {
        echo '3';
        die();
    }
    ...
    wp_update_user(array('ID' => $user_id, 'display_name' => $params['cand_name'], 'first_name' => $params['cand_first_name'], 'last_name' => $params['cand_last_name']));
    ...
    $cand_cover_letter = isset($params['cand_cover_letter']) ? $params['cand_cover_letter'] : "";
    $custom_field_data = $params['_custom_'];
    ...
    update_user_meta($user_id, '_cand_cover_letter', $cand_cover_letter);
    ...
    update_user_meta($user_id, 'can_custom_fields_array', $custom_field_data);
    ...
    if (isset($params['cand_country_towns']) && $params['cand_country_towns'] != "") {
...
/* * ********************************* */
/* Ajax handler for Make Default Candidate Resume */
/* * ********************************* */
add_action('wp_ajax_make_default_resume', 'nokri_make_default_resume');
if (!function_exists('nokri_make_default_resume')) {

    function nokri_make_default_resume() {
        global $nokri;
        $user_id = get_current_user_id();
        $make_default_resume = $_POST['data_id'];
        /* demo check */
        $is_demo = nokri_demo_mode();
        if ($is_demo) {
            echo '0|' . esc_html__("Edit in demo user not allowed", 'nokri');
            ...
            update_user_meta($user_id, '_cand_make_default_resume', $make_default_resume);
        }
        echo '1';
        die();
    }
            ...
            foreach ($files['name'] as $key => $value) {
                if ($files['name'][$key]) {
                    ...
                    if ($imageFileType != "pdf" && $imageFileType != "doc" && $imageFileType != "docx"
                        ...
                        echo '1|' . esc_html__("Sorry, only PDF, DOC, DOCX  files are allowed.", 'nokri');
                        die();
                    }
                    /* Check file size */
                    $size_arr = explode('-', $nokri['sb_upload_resume_size']);
...
if (!function_exists('nokri_delete_cand_resume')) {
    ...
    function nokri_delete_cand_resume() {
        if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'del_my_resume_nonce_action')) {
            echo '3';
            die();
        }
        $user_crnt_id = get_current_user_id();
...
if (!function_exists('nokri_aplly_job')) {
    ...
    function nokri_aplly_job() {
        ...
        if ($is_apply_pkg_base == '1' && $user_type != '1' && is_user_logged_in()) {
                            ...
                            <div class="modal-body">
                                <div class="col-md-6 col-sm-12">
                                    <div class="form-group">
                                        <label>' . esc_html__('Your Name', 'nokri') . '<span class="color-red">*</span></label>
                                        <input placeholder="' . esc_attr__('Enter your name', 'nokri') . '" class="form-control" type="text" name="sb_reg_name"  data-parsley-required="true" data-parsley-error-message="' . esc_attr__('Please enter your name', 'nokri') . '">
                                    </div>
                                </div>
                                <div class="col-md-6 col-sm-6 ">
                                    <div class="form-group">
                                        <label>' . esc_html__('Your Email', 'nokri') . '<span class="color-red">*</span></label>
                                        <input placeholder="' . esc_attr__('Enter your email address', 'nokri') . '" class="form-control" type="email" data-parsley-type="email" data-parsley-required="true"  data-parsley-error-message="' . esc_attr__('Please enter your valid email', 'nokri') . '" data-parsley-trigger="change" name="sb_reg_email">
                                    </div>
                                </div>
                                <div class="col-md-12 col-sm-12 ">
                                    <div class="form-group">
                                        <label>' . esc_html__('Upload Resume', 'nokri') . '<span class="color-red">*</span></label>
                                        <input id="input-b2"  name="my_file_upload[]" type="file" class="file sb_files-data-doc" data-show-preview="false" data-show-upload="false" data-parsley-required="true"  data-parsley-error-message="' . esc_attr__('Upload resume', 'nokri') . '" data-msg-placeholder="' . esc_attr__("Click to upload", "nokri") . '"  >
                                ...
                                <div class="col-md-12 col-sm-12 ">
                                    <div class="form-group">
                                        ...
                                        <textarea name="cand_cover_letter" rows="6" class="form-control job_textarea" placeholder="' . esc_attr__("Write cover letter", "nokri") . '" ></textarea>
                                    </div></div>
                                <div class="col-md-12 col-sm-12 ">
                                    <div class="form-group">
                                    '. wp_nonce_field("apply_now_nonce_action", "apply_now_nonce") .'
        ...
        if ($user_id == '' || $user_id == 0 ) {
            // echo '2';
            echo '2|' . esc_url(get_the_permalink($signin_page));
        ...
        if (get_user_meta($user_id, '_cand_resume', true) != "") {
            ...
            $cand_cover_letter_det = get_user_meta($user_id, '_cand_cover_letter', true);
            $defaultCv = get_user_meta($user_id, '_cand_make_default_resume', true);
            $resume_options = '';
            foreach ($resumes as $resum) {
                $selected = ($resum == $defaultCv) ? 'selected' : '';
                $resume_options .= '<option value="' . $resum . '"' . $selected . '>' . basename(get_attached_file($resum)) . '</option>  ';
            }
            $select_resumes = '<div class="form-group ' . $hide_resume_opt . '">
							<label>' . $select_resume_txt . '</label>
							<select class="select-generat " data-parsley-required=' . $parsely_req . '  data-parsley-error-message="' . esc_attr__('Select your resume to apply', 'nokri') . '" name="cand_apply_resume">
							...
							<div class="form-group">
							<label>' . esc_html__('Write cover letter (optional)', 'nokri') . '</label>
								<textarea name="cand_cover_letter" rows="6" class="form-control job_textarea" placeholder="' . esc_attr__("Write cover letter", "nokri") . '" >'. $cand_cover_letter_det .'</textarea>
							</div>
						</div>
						<div class="modal-footer">
                        '. wp_nonce_field("apply_now_nonce_action", "apply_now_nonce") .'
						  <button type="submit" name="submit" class="btn n-btn-flat btn-mid btn-block" id="submit_cv_form_btn">' . esc_attr__('Apply now', 'nokri') . '</button>
						</div>
						<input type="hidden" name="current_job"   id="current_job" value="' . esc_attr($job_id) . '" />
						<input type="hidden" name="current_author" id="current_author" value="' . esc_attr($author_id) . '" />';
        } else {
            $select_resumes = '<div class="form-group ' . $hide_resume_opt . '">
								...
								<input id="input-b2"  name="candidate_resume[]" type="file" data-parsley-required=' . $parsely_req . ' class="file upload_resume_now" data-show-preview="false" data-show-upload="false"   data-parsley-error-message="' . esc_attr__('Upload resume', 'nokri') . '" data-msg-placeholder="' . esc_attr__("Click to upload", "nokri") . '">
							</div>
							' . $questions_html . '
		                    <div class="form-group">
					          <label>' . esc_html__('Write cover letter (optional)', 'nokri') . '</label>
                              <textarea name="cand_cover_letter" rows="6" class="form-control job_textarea" placeholder="' . esc_attr__("Write cover letter", "nokri") . '">
							  </textarea>
                             </div>
...
if (!function_exists('nokri_aplly_job_with_email')) {
    ...
    function nokri_aplly_job_with_email() {
        ...
        if ($is_apply_pkg_base == '1' && $user_type != '1') {
            $is_package = nokri_candidate_package_expire_notify();
            if ($is_package == 'ae' || $is_package == 'pe' || $is_package == 'np') {
                echo '6|' . esc_html__("Please Purchase Package", 'nokri') . '|' . get_the_permalink($cand_package_page);
                die();
            }
        }
        /* demo check */
        ...
        if (get_user_meta($user_id, '_cand_resume', true) != "") {
            ...
            $resume_options = '';
            foreach ($resumes as $resum) {
                $resume_options .= '<option value="' . esc_attr($resum) . '">' . basename(get_attached_file($resum)) . '</option>  ';
            }
            $select_resumes = '<div class="form-group">
							<label>' . esc_html__('Select your resume to apply', 'nokri') . '</label>
							<select class="select-generat" data-allow-clear="true" data-parsley-required="true" data-parsley-error-message="' . esc_attr__('Select your resume to apply', 'nokri') . '" name="cand_apply_resume">
									<option value="">' . esc_html__('Select your resume', 'nokri') . '</option>
									' . $resume_options . '
								</select>
							</div>
						...
						<div class="modal-footer">
						  <button type="submit" name="submit" class="btn n-btn-flat btn-mid btn-block" id="submit_cv_form_email_btn">' . esc_attr__('Apply now', 'nokri') . '</button>
						</div>
						<input type="hidden" name="current_job"   id="current_job" value="' . esc_attr($job_id) . '" />
						<input type="hidden" name="current_author" id="current_author" value="' . esc_attr($author_id) . '" />';
        } else {
            $select_resumes = '<div class="form-group">
								...
								<input id="input-b2"  name="candidate_resume[]" type="file" class="file upload_resume_now" data-show-preview="false" data-show-upload="false" data-parsley-required="true"  data-parsley-error-message="' . esc_attr__('Upload resume', 'nokri') . '" data-msg-placeholder="' . esc_attr__("Click to upload", "nokri") . '">
							</div>
							' . $questions_html . '
		                    <div class="form-group">
					          <label>' . esc_html__('Write cover letter (optional)', 'nokri') . '</label>
...
// Allow certain file formats
                    ...
                    if ($imageFileType != "pdf" && $imageFileType != "doc" && $imageFileType != "docx"
                        ...
                        echo '0|' . esc_html__("Sorry, only PDF, DOC, DOCX  files are allowed.", 'nokri');
                        die();
                    }
                    /* Check file size */
                    $size_arr = explode('-', $nokri['sb_upload_resume_size']);
            ...
            <div class="modal-dialog modal-lg">
              ...
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title">' . esc_html__('Take action on', 'nokri') . " " . esc_html($candidate_info->display_name) . " " . esc_html__('application', 'nokri') . '</h4>
                </div>
                <div class="modal-body">
                    	...
                    	<div class="row">
                        	<div class="company-search-toggle">
                                ...
                                <div class="col-md-3  col-sm-3">
                                    <div class="float-end ' . esc_attr($rtl_class) . '">
                                      <input id="email_send_toggle"  data-on="' . esc_html__('YES', 'nokri') . '" data-off="' . esc_html__('NO', 'nokri') . '" data-size="small" data-bs-toggle="toggle" type="checkbox">
...
// Getting values From Param
        ...
        if (empty($params['submit_linkedin_url_nonce']) || !wp_verify_nonce($params['submit_linkedin_url_nonce'], 'submit_linkedin_url_nonce_action')) {
            echo "3";
            die();
        }
        ...
        echo esc_url(get_the_permalink($job_id));
        die();
    }
...
// Getting values From Param
        ...
        if(empty($params['apply_now_nonce']) || !wp_verify_nonce($params['apply_now_nonce'], 'apply_now_nonce_action')) {
            echo "5";
            die();
        }
...
if (!function_exists('nokri_resume_access_package_update')) {
    ...
    function nokri_resume_access_package_update() {
        ...
        if (current_user_can('administrator') || $package_base == '1') {
            ...
            $final_url = esc_url(nokri_page_lang_url_callback($link));
            echo '4|' . esc_url($final_url . '&download_file=1');
            die();
        }
        if ($package_base == '2') {
            ...
            if ($can_download == 'ae') {
                echo '1|' . esc_html__("Please purchase package", 'nokri') . '|' . esc_url(get_permalink($package_page));
                die();
            } else if ($can_download == 'pe') {
                echo '2|' . esc_html__("Please purchase package", 'nokri') . '|' . esc_url(get_permalink($package_page));
                die();
            } else if ($can_download == 'np') {
                echo '3|' . esc_html__("Please purchase package", 'nokri') . '|' . esc_url(get_permalink($package_page));
                die();
            } else if ($can_download == 'en') {
                echo '4|' . esc_url(get_permalink($attach_id) . '?attachment_id=' . $attach_id . '&download_file=1');
                die();
            } else {
...
if (!function_exists('nokri_save_my_job')) {
    ...
    function nokri_save_my_job() {
        global $nokri;
        if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'save_job_nonce_action')) {
            echo '8';
            die();
        }
        /* demo check */
...
if (!function_exists('nokri_del_saved_job')) {
    ...
    function nokri_del_saved_job() {
        global $nokri;
        if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'del_saved_job_nonce_action')) {
            echo '3';
            die();
        }
        $user_id = get_current_user_id();
...
if (!function_exists('nokri_following_company')) {
    ...
    function nokri_following_company() {
        ...
        if ($user_id == 0) { // User not logged in
            echo '2'; // You can return 2 to indicate that the user is not logged in
            die();
        }
        /* demo check */
        $is_demo = nokri_demo_mode();
...
if (!function_exists('nokri_un_following_company')) {
    ...
    function nokri_un_following_company() {
        global $nokri;
        if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'unfollow_comp_nonce_action')) {
            echo '3';
            die();
        }
        $user_id = get_current_user_id();
...
if (!function_exists('nokri_applied_jobs')) {

    function nokri_applied_jobs($user_id) {
        /* Query For Getting All Followed Companies */
        global $wpdb;
        $query = "SELECT meta_value FROM $wpdb->usermeta WHERE user_id = '$user_id' AND meta_key like '_job_applied_resume_%'";
        $cand_followings = $wpdb->get_results($query);
        if (count((array) $cand_followings) > 0) {
            $ids = array();
            foreach ($cand_followings as $companies) {
...
// Check file size
                    /* if ($file['size'] > 2097152) {
                      echo '0|' . esc_html__( "Max allowd image size is 2MB", 'nokri' );
                      die();
                      } */
...
// Getting values From Param
        ...
        if(empty($params['job_alerts_nonce']) || !wp_verify_nonce($params['job_alerts_nonce'], 'job_alerts_nonce_action')) {
            echo '5';
            die();
        }
...
if (!function_exists('nokri_job_alert_paid_subscription')) {
    ...
    function nokri_job_alert_paid_subscription() {
        ...
        if ($woocommerce->cart->add_to_cart($product_id, 1)) {
            echo '1|' . esc_html__("Added to cart.", 'nokri') . '|' . wc_get_cart_url();
        } else {
            echo '1|' . esc_html__("Already in your cart.", 'nokri') . '|' . wc_get_cart_url();
        }
        die();
    }
...
if (!function_exists('nokri_del_job_alerts')) {
    ...
    function nokri_del_job_alerts() {
        global $nokri;
        if(empty($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'del_save_alert_nonce_action')) {
            echo '3';
            die();
        }
        $user_id = get_current_user_id();
...
if (!function_exists('nokri_external_apply_package_base')) {
    ...
    function nokri_external_apply_package_base() {
        ...
        $is_demo = nokri_demo_mode();
        if ($is_demo) {
            echo '1|' . esc_html__("Demo mode activated", 'nokri');
            die();
        }
        /* Is applying job package base */
        $is_apply_pkg_base = ( isset($nokri['job_apply_package_base']) && $nokri['job_apply_package_base'] != "" ) ? $nokri['job_apply_package_base'] : false;
        ...
        if ($is_apply_pkg_base == '1') {
            ...
            if ($is_apply_pkg_base == '1' && $user_id == '') {
                echo '2|' . esc_html__("Please Login To Apply ", 'nokri') . '|' . get_the_permalink($sign_in);
                die();
            }
            /* Cand package page */
            $cand_package_page = ( isset($nokri['cand_package_page']) && $nokri['cand_package_page'] != "" ) ? $nokri['cand_package_page'] : '';
            ...
            if ($is_apply_pkg_base == '1' && $user_type != '1') {
                $is_package = nokri_candidate_package_expire_notify();
                if ($is_package == 'ae' || $is_package == 'pe' || $is_package == 'np') {
                    echo '3|' . esc_html__("Please Purchase Package", 'nokri') . '|' . get_the_permalink($cand_package_page);
            ...
            if ($with_log == '1' && $user_id == '') {
                echo '2|' . esc_html__("Please Login To Apply ", 'nokri') . '|' . esc_url(get_the_permalink($sign_in));
                die();
            } else {
                echo '4';
                die();
                            ...
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-lg-6 col-md-3  col-sm-3">                                    
                                        ...
                                        <input class="form-control" type="text" name="meeting_title" id ="meeting_title" required="" value="<?php echo esc_attr($meeting_topic); ?>" data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('Meeting Title', 'nokri'); ?>">      
                                    </div>
                                    <div class="col-lg-6 col-md-3  col-sm-3"> 
                                        <label><?php echo esc_html__('Meeting Date:', 'nokri'); ?></label> 
                                        <input class="form-control"  type="date" name="meeting_date" id ="meeting_date" required="" value="<?php echo esc_attr($meeting_date); ?>" data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('Meeting Date', 'nokri'); ?>">
                                    </div>
                                    <div class="col-lg-6 col-md-3  col-sm-3"> 
                                        <label><?php echo esc_html__('Meeting Time:', 'nokri'); ?></label> 
                                        <input class="form-control" type="time" name="meeting_time" id ="meeting_time" required="" value="<?php echo esc_attr($meeting_time); ?>" data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('Meeting Time', 'nokri'); ?>">
                                    </div>
                                    <div class="col-lg-6 col-md-3  col-sm-3"><label><?php echo esc_html__('Meeting Duration', 'nokri'); ?></label> 
                                        <input  value="<?php echo esc_attr($meeting_duration); ?>" class="form-control account-members" required="" type="text" name="meeting_duration" id="meeting_duration"  data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('Meeting Duration', 'nokri'); ?>"></div>        

                                    <div class="col-lg-6 col-md-3  col-sm-3"> <label><?php echo esc_html__('Meeting Password', 'nokri'); ?></label> 
                                        <input class="form-control" value="<?php echo esc_attr($meeting_password); ?>" required="" type="text" name="meeting_password"  data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('Meeting Password', 'nokri'); ?>"></div>    

                                    <div class="col-lg-6 col-md-3  col-sm-3"> <label><?php echo esc_html__('Meeting Note', 'nokri'); ?></label> 
                                        <input class="form-control" value="<?php echo esc_attr($meeting_note); ?>" required="" type="textarea" rows="4" cols="50" name="meeting_note"  data-parsley-required="true" data-parsley-error-message="<?php echo esc_attr__('Meeting Note', 'nokri'); ?>"></div>    
                                </div>
                            </div>
                            <div class="modal-footer"> 
...
if (!function_exists('nokri_setup_zoon_meeting_func')) {
    ...
    function nokri_setup_zoon_meeting_func() {
        ...
        if ($user_id->ID != is_author()) {
            ...
            if ($meeting_id != "") {
                $url = esc_url('https://api.zoom.us/v2/meetings/' . $meeting_id);
                $data['id'] = $meeting_id;
            } else {
                $url = esc_url('https://api.zoom.us/v2/users/me/meetings');
            }
...
if (!function_exists('nokri_zoom_delete_meet')) {
    ...
    function nokri_zoom_delete_meet() {
        ...
        $is_demo = nokri_demo_mode();
        if ($is_demo) {
            echo '0|' . esc_html__("You can not delete because dome mode is enabled.", "nokri");
            die();
        }
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <h4><?php echo esc_html($form_field_description); ?></h4>
                        <div class="form-group">
...
if (!function_exists('nokri_job_report_submission')) {
    ...
    function nokri_job_report_submission() {
        ...
        if ($reason_msg == '') {
            echo '1|' . esc_html__("Select atleast one Reason to report this job", "nokri");
            die();
        }
        /* getting and updating report description */
        if ($description == '') {
            echo '1|' . esc_html__("Please write a short description to report this job", "nokri");
            die();
        }

        $data_array = array();
        if ($reason_msg != "") {
            $data_array['reason'] = $reason_msg;
        }
        if ($description != "") {
            $data_array['desc'] = $description;
        }
        $data_array = !empty($data_array) ? json_encode($data_array) : "";

        /* Check if User Already reported this Job */
        $is_reported = get_post_meta($post_id, "_sb_job_report_user_" . $user_id, true);

        if ($is_reported != '') {
            echo '1|' . esc_html__("Sorry! You have alraedy reported this Job", "nokri");
            die();
        }
        /* getting and updating report user_id */
        if ($report_user_id && $post_id > 0 && $description && $reason_msg != '') {
            update_post_meta($post_id, "_sb_job_report_user_" . $user_id, $data_array);
        } else {
            echo '0|' . esc_html__("You are not allowed to do this action", "nokri");
            die();
        }
        ...
        $report_description = esc_attr($description);
        $author_job_title = get_the_title($post_id);
        if ($nokri['sb_new_job_report_email_btn'] == '1' && isset($nokri['_cand_job_report_subject']) && $nokri['_cand_job_report_subject'] != "") {
            // Auhtor info
            $subject = $nokri['_cand_job_report_subject'];
            $headers = array('Content-Type: text/html; charset=UTF-8', "From: $reporter_email");
            $msg_keywords = array('%site_name%', '%job_title%', '%candidate_name%', '%candidate_link%', '%job_link%', '%reason_msg%', '%description%');
            $msg_replaces = array(get_bloginfo('name'), $author_job_title, $reporter_name, $user_login, $job_link, $reason_msg, $report_description);
            $body = str_replace($msg_keywords, $msg_replaces, $nokri['sb_new_report_email_template']);
            ...
            if ($reports_counter >= $no_of_job_reported) {
                $body = str_replace($msg_keywords, $msg_replaces, $limits_message, $nokri['sb_new_report_email_template']);
                wp_mail($admin_email, $subject, $body, $headers);
                echo '1|' . esc_html__("This job is already reported and under admin reviews, Thanks!", "nokri");
                die();
            }
            /* Report Submission Mail to the admin */
            $sent_message = wp_mail($admin_email, $subject, $body, $headers);
            if (!$sent_message) {
                echo '0|' . esc_html__("Sorry! You got some Errors", "nokri");
            } else {
                echo '1|' . esc_html__("Your request has been submitted successfully", "nokri");
            }
            if ($nokri['sb_report_email_cand_btn'] == '1' && isset($nokri['sb_cand_job_report_subject']) && $nokri['sb_cand_job_report_subject'] != "" && isset($nokri['sb_admin_email_report']) && $nokri['sb_admin_email_report'] != "") {

                $subject = $nokri['sb_cand_job_report_subject'];
                $headers = array('Content-Type: text/html; charset=UTF-8', "From: $admin_email");
                $msg_keywords = array('%site_name%', '%job_title%', '%candidate_name%', '%candidate_link%', '%job_link%', '%reason_msg%', '%description%');
                $msg_replaces = array(get_bloginfo('name'), $author_job_title, $reporter_name, $user_login, $job_link, $reason_msg, $report_description);
                $body = str_replace($msg_keywords, $msg_replaces, $nokri['sb_cand_report_email_template']);

                /* Report Submission Mail to the User */
                wp_mail($reporter_email, $subject, $body, $headers);
            }
        }
