...
function feilds_for_html($post) {
    ...
    <p>
        ...
        <select name="custom_feild_for" id="custom_feild_for">
            <option <?php echo ($custom_feilds_for === 'Registration' ) ? 'selected=selected' : '' ?> value="Registration"><?php echo __('Registration', 'redux-framework'); ?></option>
            <option <?php echo ($custom_feilds_for === 'Candidate' ) ? 'selected=selected' : '' ?> value="Candidate"><?php echo __('Candidate', 'redux-framework'); ?></option>
            <option <?php echo ($custom_feilds_for === 'Employer' ) ? 'selected=selected' : '' ?> value="Employer"><?php echo __('Employer', 'redux-framework'); ?></option>
        </select>
    </p><?php
}
...
function feilds_for_save($post_id) {
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)
        return;
    if (!isset($_POST['feilds_for_nonce']) || !wp_verify_nonce($_POST['feilds_for_nonce'], '_feilds_for_nonce'))
        return;
    ...
    <div id="custom_feilds_wrapper">
        ...
        if (is_array($custom_feilds)) {
            foreach ($custom_feilds as $value) {
                ...
                $slug_field = isset($value->feild_slug) ? $value->feild_slug : "";
                ?>
                <div class="custom_feilds_repeats" id="custom_repeat<?php echo $count; ?>">
                    <p>
                        ...
                        <select name="custom_feilds_select_an_option[]" id="custom_feilds_select_an_option" class="select_feild_type">
                            ...
                            <option <?php echo ($value->feild_type === 'Input' ) ? 'selected=selected' : '' ?> value="Input"><?php echo __('Input', 'redux-framework'); ?></option>
                            <option <?php echo ($value->feild_type === 'RadioButton' ) ? 'selected=selected' : '' ?> value="RadioButton"><?php echo __('RadioButton', 'redux-framework'); ?></option>
                            <option <?php echo ($value->feild_type === 'Select Box' ) ? 'selected=selected' : '' ?> value="Select Box"><?php echo __('Select Box', 'redux-framework'); ?></option>
                            <option <?php echo ($value->feild_type === 'Text Area' ) ? 'selected=selected' : '' ?> value="Text Area"><?php echo __('Text Area', 'redux-framework'); ?></option>
                            <option <?php echo ($value->feild_type === 'Date' ) ? 'selected=selected' : '' ?> value="Date"><?php echo __('Date', 'redux-framework'); ?></option>
                            <option <?php echo ($value->feild_type === 'Number' ) ? 'selected=selected' : '' ?> value="Number"><?php echo __('Number', 'redux-framework'); ?></option>
                        </select>
                    </p>
                    <p class="custom_feilds_labels">
                        <label for="custom_feilds_labels"><?php _e(' Enter Field Label', 'redux-framework'); ?></label><br>
                        <input type="text" name="custom_feilds_label[]" value="<?php echo $value->feild_label; ?>" required="required"  class="sb-get-tilte">

                        <input type="text" class="custom_feilds_slug sb-get-slug" name="custom_feilds_slug[]" value="<?php echo '' . $slug_field; ?>" required="required" readonly="">

                        <input type="Checkbox" class="sb-get-slug-edit">
                        <strong><?php echo esc_attr(__("Edit", "redux-framework")); ?></strong> <span><?php echo esc_attr(__("Enter the slug name, it must be unique and chage it with causion. only alpha numeric and _", "redux-framework")); ?></span>
                    </p>
...
function custom_feilds_save($post_id) {
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE)
        return;
    if (!isset($_POST['custom_feilds_nonce']) || !wp_verify_nonce($_POST['custom_feilds_nonce'], '_custom_feilds_nonce'))
        return;
    if (!current_user_can('edit_post', $post_id))
        return;
    ...
    for ($i = 0; $i < count($_POST['custom_feilds_select_an_option']); $i++) {
        ...
        $feilds[] = array(
            ...
            'feild_slug' => $_POST['custom_feilds_slug'][$i],
...
function nokri_admin_scripts_enqueue_custom_feilds() {
    ...
    <script type="text/javascript">
        ...
        jQuery(".sb-get-slug-edit").on("click", function () {

            var checkedval = jQuery(this).is(':checked');
            if (checkedval)
            {
                jQuery(this).closest('.custom_feilds_labels').find('input.sb-get-slug').removeAttr('readonly');

            } else
            {
                jQuery(this).closest('.custom_feilds_labels').find('input.sb-get-slug').attr('readonly', 'readonly');
            }
        });
        ...
        jQuery(document).on("change", ".sb-get-tilte", function (e) {

            var isData = jQuery(this).parent().find('input.sb-get-slug').val();
            if (isData === "")
            {
                var selectVal = jQuery(this).val();
                ...
                jQuery(this).parent().find('input.sb-get-slug').val(slugVal);
            }
        });
    </script>
    <?php
